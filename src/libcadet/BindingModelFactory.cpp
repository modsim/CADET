...
void registerMultiStateStericMassActionModel(
	std::unordered_map<std::string, std::function<model::IBindingModel*()>>& bindings);
//			void registerSimplifiedMultiStateStericMassActionModel(std::unordered_map<std::string,
// std::function<model::IBindingModel*()>>& bindings);
...
template <class BindingModel_t> void BindingModelFactory::registerModel(const std::string& name)
{
	_bindingModels[name] = []() { return new BindingModel_t(); };
}

template <class BindingModel_t> void BindingModelFactory::registerModel()
{
...
void BindingModelFactory::registerModel(const std::string& name, std::function<model::IBindingModel*()> factory)
{
	if (_bindingModels.find(name) == _bindingModels.end())
		...
		throw InvalidParameterException("IBindingModel implementation with the name " + name +
