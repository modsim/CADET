...
public:
	...
	/**
	 * @brief Configures the model discretization by extracting all structural parameters from the given @p
	 * paramProvider
	...
	/**
	 * @brief (Re-)configures the model by extracting all non-structural parameters (e.g., model parameters) from the
	 * given @p paramProvider
	 * @details The scope of the cadet::IParameterProvider is left unchanged on return.
	 *
	...
	/**
	 * @brief (Re-)configures a unit operation model by extracting its non-structural parameters (e.g., model
	 ...
	 *          time integrator.
	 *
	 * @param [in,out] simState State of the simulation (state vector and its time derivatives) to be updated with
	 * initial values
	 */
	 ...
	 *          system reads \f[ \frac{\partial F}{\partial y}(t, y, \dot{y}) s + \frac{\partial F}{\partial \dot{y}}(t,
	 * y, \dot{y}) \dot{s} + \frac{\partial F}{\partial p}(t, y, \dot{y}) = 0. \f] The initial value of \f$ s_0 =
	 * \frac{\mathrm{d} y_0}{\mathrm{d}p} \f$ is set by this function. Note that consistent initialization is performed
	 * later, which in particular calculates a matching \f$ \dot{s}_0 = \frac{\mathrm{d} \dot{y}_0}{\mathrm{d}p}. \f$
	 *
	 ...
	 * @param [in,out] simState State of the simulation (state vector and its time derivatives) with initial values that
	 * are to be updated for consistency
	 * @param [in,out] adJac Jacobian information for AD (AD vectors for residual and state, direction offset)
	...
	virtual void consistentInitialConditions(const SimulationTime& simTime, const SimulationState& simState,
											 const AdJacobianParams& adJac, double errorTol) = 0;
	 ...
	 * \dot{s} + \frac{\partial F}{\partial p}(t, y, \dot{y}) = 0. \f] The initial values of \f$ s_0 = \frac{\mathrm{d}
	 * y_0}{\mathrm{d}p} \f$ and \f$ \dot{s}_0 = \frac{\mathrm{d} \dot{y}_0}{\mathrm{d}p} \f$ have to be consistent,
	 * that means, they have to satisfy the sensitivity equation. This function computes the correct \f$ s_0 \f$ and \f$
	 * \dot{s}_0 \f$ given \f$ y_0 \f$ and \f$ s_0 \f$.
	 *
	 * @param [in] simTime Simulation time information (time point, section index, pre-factor of time derivatives)
	 ...
	 * @param [in,out] adY Pointer to global state vector of AD datatypes that can be used for computing the Jacobian
	 * (or @c nullptr if AD is disabled)
	 */
	virtual void consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
											  std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot,
											  active* const adRes, active* const adY) = 0;
	...
	virtual int residualWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState,
									 double* const res, const AdJacobianParams& adJac) = 0;
