...
namespace
{
template <class T> const std::vector<T> convertNVectorToStdVectorPtrs(N_Vector* vec, unsigned int numVec)
{
...
const std::vector<const double*> convertNVectorToStdVectorConstPtrs(unsigned int& len, N_Vector* vec,
																	unsigned int numVec)
{
	if (!vec || (numVec == 0))
	{
...
{
	return ((id.name == cadet::hashString("SECTION_TIMES")) && (id.section < nSectionTimes) &&
			(id.component == cadet::CompIndep) && (id.particleType == cadet::ParTypeIndep) &&
			(id.boundState == cadet::BoundStateIndep) && (id.reaction == cadet::ReactionIndep) &&
 ...
 */
inline cadet::ConsistentInitialization currentConsistentInitMode(cadet::ConsistentInitialization mode,
																 unsigned int curSec)
{
	switch (mode)
	{
...
void idasErrorHandler(int error_code, const char* module, const char* function, char* msg, void* eh_data)
{
	...
	oss << "In function '" << function << "' of module '" << module << "', error code '"
		<< getIDAReturnFlagName(error_code) << "':\n"
		<< msg;
...
int residualDaeWrapper(double t, N_Vector y, N_Vector yDot, N_Vector res, void* userData)
{
	...
	return sim->_model->residualWithJacobian(
		cadet::SimulationTime{t, secIdx}, cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)}, NVEC_DATA(res),
...
int linearSolveWrapper(IDAMem IDA_mem, N_Vector rhs, N_Vector weight, N_Vector y, N_Vector yDot, N_Vector res)
{
	...
	return sim->_model->linearSolve(t, alpha, tol, NVEC_DATA(rhs), NVEC_DATA(weight),
									cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)});
}
						...
						N_Vector* resS, void* userData, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
	...
	/*
			reinterpret_cast<cadet::model::ModelSystem*>(sim->_model)->genJacobian(t, secIdx, NVEC_DATA(y),
	   NVEC_DATA(yDot)); reinterpret_cast<cadet::model::ModelSystem*>(sim->_model)->genJacobian(ns, t, NVEC_DATA(y),
	   NVEC_DATA(yDot), NVEC_DATA(res), sensY, sensYdot, sensRes, sim->_vecADres, NVEC_DATA(tmp1), NVEC_DATA(tmp2),
	   NVEC_DATA(tmp3));
	*/

	return sim->_model->residualSensFwd(
		ns, cadet::SimulationTime{t, secIdx}, cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)},
		NVEC_DATA(res), sensY, sensYdot, sensRes, sim->_vecADres, NVEC_DATA(tmp1), NVEC_DATA(tmp2), NVEC_DATA(tmp3));
}

Simulator::Simulator()
	: _model(nullptr), _solRecorder(nullptr), _idaMemBlock(nullptr), _vecStateY(nullptr), _vecStateYdot(nullptr),
	  _vecFwdYs(nullptr), _vecFwdYsDot(nullptr), _relTolS(1.0e-9), _absTol(1, 1.0e-12), _relTol(1.0e-9),
	  _initStepSize(1, 1.0e-6), _maxSteps(10000), _maxStepSize(0.0), _nThreads(0), _sensErrorTestEnabled(true),
	  _maxNewtonIter(3), _maxErrorTestFail(7), _maxConvTestFail(10), _maxNewtonIterSens(3), _curSec(0),
	  _skipConsistencyStateY(false), _skipConsistencySensitivity(false),
...
void Simulator::updateMainErrorTolerances()
{
	...
	if (_absTol.size() > 1)
	{
		...
		if (_absTol.size() >= pureDofs)
		{
			...
			const std::vector<double> addAbsErrTol =
				_model->calculateErrorTolsForAdditionalDofs(_absTol.data(), _absTol.size());
...
void Simulator::applyInitialConditionFwdSensitivities(double const* const* const initSens,
...
std::unordered_map<ParameterId, double> Simulator::getAllParameterValues() const
{
	...
	for (std::size_t i = 0; i < _sectionTimes.size(); ++i)
		data[makeParamId(secTimesName, UnitOpIndep, CompIndep, ParTypeIndep, BoundStateIndep, ReactionIndep, i)] =
			static_cast<double>(_sectionTimes[i]);
...
void Simulator::setSensitiveParameter(ParameterId const* ids, double const* diffFactors, unsigned int numParams,
									  double absTolS)
{
	// Set AD directions
...
bool Simulator::setSectionTimesSensitive(const ParameterId& id, unsigned int adDirection, double adValue)
{
	...
	{
		// Correct adValue
		LOG(Debug) << "Found parameter " << id << " in SECTION_TIMES: Dir " << adDirection << " is set to " << adValue
				   << " [" << static_cast<double>(_sectionTimes[id.section]) << " < " << _sectionTimes.size() << "]";
	...
	for (std::size_t i = 0; i < sectionTimes.size() - 1; ++i)
		if (sectionTimes[i] > sectionTimes[i + 1])
			throw InvalidParameterException(
				"The end time of each section must be greater than its start time (failed for section " +
				std::to_string(i) + ")!");
...
void Simulator::integrate()
{
	...
	tbb::global_control tbbGlobalControl(tbb::global_control::max_allowed_parallelism,
										 (_nThreads > 0) ? _nThreads : tbb::this_task_arena::max_concurrency());
#else
...
#if defined(ACTIVE_SFAD) || defined(ACTIVE_SETFAD)
	LOG(Debug) << "Setting AD directions from " << ad::getDirections() << " to "
			   << numSensitivityAdDirections() + _model->requiredADdirs();
	if (numSensitivityAdDirections() + _model->requiredADdirs() > ad::getMaxDirections())
		throw InvalidParameterException(
			"Requested " + std::to_string(numSensitivityAdDirections() + _model->requiredADdirs()) +
			" AD directions, but only " + std::to_string(ad::getMaxDirections()) + " are supported");
	...
	LOG(Debug) << "#MaxNewton: " << _maxNewtonIter << ", #MaxErrTestFail: " << _maxErrorTestFail
			   << ", #MaxConvTestFail: " << _maxConvTestFail;
	if (wantSensitivities)
	{
		LOG(Debug) << "Sensitvities in error test: " << _sensErrorTestEnabled
				   << ", #MaxNewtonSens: " << _maxNewtonIterSens;
	}
	...
	LOG(Debug) << "Integration span: [" << static_cast<double>(_sectionTimes[0]) << ", "
			   << static_cast<double>(_sectionTimes.back()) << "] sections";
	...
	while (curT < tEnd)
	{
		...
		const double endTime = writeAtUserTimes ? std::min(static_cast<double>(_sectionTimes[_curSec + skip]), tEnd)
												: static_cast<double>(_sectionTimes[_curSec + skip]);
		curT = startTime;
		...
		const double consPrev = _model->residualNorm(
			SimulationTime{curT, _curSec}, ConstSimulationState{NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot)});
		...
		if (!_skipConsistencyStateY && (_consistentInitMode != ConsistentInitialization::None))
		{
			...
			{
				_model->consistentInitialConditions(
					SimulationTime{curT, _curSec}, SimulationState{NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot)},
					AdJacobianParams{_vecADres, _vecADy, numSensitivityAdDirections()}, _algTol);

				const double consPost =
					_model->residualNorm(SimulationTime{curT, _curSec},
			...
			{
				LOG(Debug) << " ==========> Consistent initialization NOT performed (mode "
						   << to_string(_consistentInitMode) << ")";
			}
		...
		{
#ifdef CADET_DEBUG
			const std::vector<const double*> sensYdbg =
				convertNVectorToStdVectorPtrs<const double*>(_vecFwdYs, _sensitiveParams.slices());
			const std::vector<const double*> sensYdotDbg =
				convertNVectorToStdVectorPtrs<const double*>(_vecFwdYsDot, _sensitiveParams.slices());

			std::vector<double> norms(_sensitiveParams.slices(), 0.0);
			std::vector<double> temp(_model->numDofs(), 0.0);
			_model->residualSensFwdNorm(_sensitiveParams.slices(), SimulationTime{curT, _curSec},
										ConstSimulationState{NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot)}, sensYdbg,
										sensYdotDbg, norms.data(), _vecADres, temp.data());
			...
			if (mode == ConsistentInitialization::Full)
			{
				...
				std::vector<double*> sensY =
					convertNVectorToStdVectorPtrs<double*>(_vecFwdYs, _sensitiveParams.slices());
				std::vector<double*> sensYdot =
					convertNVectorToStdVectorPtrs<double*>(_vecFwdYsDot, _sensitiveParams.slices());
				_model->consistentInitialSensitivity(
					SimulationTime{curT, _curSec},
					ConstSimulationState{NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot)}, sensY, sensYdot, _vecADres,
					_vecADy);
			...
			else if (mode == ConsistentInitialization::Lean)
			{
				...
				std::vector<double*> sensYdot =
					convertNVectorToStdVectorPtrs<double*>(_vecFwdYsDot, _sensitiveParams.slices());
				_model->leanConsistentInitialSensitivity(
					SimulationTime{curT, _curSec},
					ConstSimulationState{NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot)}, sensY, sensYdot, _vecADres,
					_vecADy);
			...
			{
				LOG(Debug) << " ==========> Sens consistent initialization NOT performed (mode "
						   << to_string(_consistentInitModeSens) << ")";
			}
			...
			{
				LOG(Debug) << "sensY[" << j
						   << "] = " << log::VectorPtr<double>(NVEC_DATA(_vecFwdYs[j]), _model->numDofs()) << ";";
				LOG(Debug) << "sensYdot[" << j
						   << "] = " << log::VectorPtr<double>(NVEC_DATA(_vecFwdYsDot[j]), _model->numDofs()) << ";";
				LOG(Debug) << "Contains NaN: sensY[" << j << "] = " << hasNaN(_vecFwdYs[j]) << " sensYdot[" << j
						   << "] = " << hasNaN(_vecFwdYsDot[j]);
			}
#endif
		}
		_skipConsistencySensitivity = false;
		...
		if (_notification)
		{
			const double progress =
				(curT - static_cast<double>(_sectionTimes[0])) / (tEnd - static_cast<double>(_sectionTimes[0]));
			if (!_notification->timeIntegrationSection(_curSec, curT, NVEC_DATA(_vecStateY), NVEC_DATA(_vecStateYdot),
													   progress))
			{
		...
		while ((solverFlag == IDA_SUCCESS) || (solverFlag == IDA_ROOT_RETURN))
		{
			...
			LOG(Debug) << "Solve from " << curT << " to " << tOut << " => "
					   << (solverFlag == IDA_SUCCESS ? "IDA_SUCCESS" : "")
			...
			case IDA_SUCCESS:
				...
				if (_notification)
				{
					const double progress =
						(curT - static_cast<double>(_sectionTimes[0])) / (tEnd - static_cast<double>(_sectionTimes[0]));
					if (!_notification->timeIntegrationStep(_curSec, curT, NVEC_DATA(_vecStateY),
															NVEC_DATA(_vecStateYdot), progress))
					{
						_lastIntTime = _timerIntegration.stop();
						return;
					}
			...
			default:
				...
				throw IntegrationException(std::string("Error in IDASolve: ") + errorFlag + std::string(" at t = ") +
										   std::to_string(curT)); // todo might not be necessary
				break;
			} // switch
...
void Simulator::configureTimeIntegrator(double relTol, double absTol, const std::vector<double>& initStepSizes,
