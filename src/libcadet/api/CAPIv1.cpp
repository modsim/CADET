...
public:
	CallbackParameterProvider(const cdtParameterProvider& pp) : _pp(pp)
	{
		...
		if (!pp.getDoubleArray && !pp.getDoubleArrayItem)
			throw InvalidParameterException(
				"ParameterProvider does neither implement getDoubleArray() nor getDoubleArrayItem()");
		if (!pp.getIntArray && !pp.getIntArrayItem)
			throw InvalidParameterException(
				"ParameterProvider does neither implement getIntArray() nor getIntArrayItem()");
		if (!pp.getBoolArray && !pp.getBoolArrayItem)
			throw InvalidParameterException(
				"ParameterProvider does neither implement getBoolArray() nor getBoolArrayItem()");
		if (!pp.getStringArray && !pp.getStringArrayItem)
			throw InvalidParameterException(
				"ParameterProvider does neither implement getStringArray() nor getStringArrayItem()");
		if (!pp.exists)
	...
	virtual std::string getString(const std::string& paramName)
	{
		...
		LOG(Debug) << "GET scalar [string] " << paramName << ": " << v << " (mem: " << static_cast<void*>(&v) << " "
	...
	virtual std::size_t numElements(const std::string& paramName)
	{
		...
		if (num < 0)
			throw InvalidParameterException("Retrieving string parameter array " + paramName +
...
private:
	...
	std::vector<double> getDoubleArrayElementwise(const std::string& paramName)
	{
		const int num = _pp.numElements(_pp.userData, paramName.c_str());
		if (num < 0)
			throw InvalidParameterException("Retrieving double parameter array " + paramName +
											" failed (does not exist)");
	...
	std::vector<bool> getBoolArrayElementwise(const std::string& paramName)
	{
		const int num = _pp.numElements(_pp.userData, paramName.c_str());
		if (num < 0)
			throw InvalidParameterException("Retrieving bool parameter array " + paramName +
											" failed (does not exist)");
	...
	std::vector<std::string> getStringArrayElementwise(const std::string& paramName)
	{
		...
		for (int i = 0; i < num; ++i)
		{
			...
			if (!temp)
				throw InvalidParameterException("Retrieving string parameter " + paramName +
												" failed (received nullptr)");
...
cdtResult getSolutionOutlet(cdtDriver* drv, int unitOpId, double const** time, double const** data, int* nTime,
							int* nPort, int* nComp)
{
	Driver* const realDrv = drv->driver;
	if (!realDrv)
