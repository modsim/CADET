...
#include "model/DeadEndFiltrationFlow.hpp"
...
DeadEndFiltrationFlow::DeadEndFiltrationFlow(UnitOpIdx unitOpIdx) : UnitOperationBase(unitOpIdx), _nComp(0), _nParType(0), _nBound(nullptr), _boundOffset(nullptr), _strideBound(nullptr), _offsetParType(nullptr), 
...
{
	// Mutliplexed binding and reaction models make no sense in DeadEndFiltrationFlow
	_singleBinding = false;
	_singleDynReaction = false;
}

DeadEndFiltrationFlow::~DeadEndFiltrationFlow() CADET_NOEXCEPT
{
	delete[] _boundOffset;
	delete[] _nBound;
	delete[] _strideBound;
...
unsigned int DeadEndFiltrationFlow::numDofs() const CADET_NOEXCEPT
{
	return 2 * _nComp + _totalBound + 1;
}

unsigned int DeadEndFiltrationFlow::numPureDofs() const CADET_NOEXCEPT
{
	return _nComp + _totalBound + 1;
}

bool DeadEndFiltrationFlow::usesAD() const CADET_NOEXCEPT
{
...
void DeadEndFiltrationFlow::setFlowRates(active const* in, active const* out) CADET_NOEXCEPT 
{ 
	_flowRateIn = in[0];
	_flowRateOut = out[0];
}

bool DeadEndFiltrationFlow::configureModelDiscretization(IParameterProvider& paramProvider, IConfigHelper& helper)
{
...
bool DeadEndFiltrationFlow::configure(IParameterProvider& paramProvider)
{
...
unsigned int DeadEndFiltrationFlow::threadLocalMemorySize() const CADET_NOEXCEPT
{
...
void DeadEndFiltrationFlow::setSectionTimes(double const* secTimes, bool const* secContinuity, unsigned int nSections)
{
}

void DeadEndFiltrationFlow::useAnalyticJacobian(const bool analyticJac)
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
void DeadEndFiltrationFlow::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx, const ConstSimulationState& simState, const AdJacobianParams& adJac)
{
...
void DeadEndFiltrationFlow::reportSolution(ISolutionRecorder& recorder, double const* const solution) const
{
...
void DeadEndFiltrationFlow::reportSolutionStructure(ISolutionRecorder& recorder) const
{
	Exporter expr(_nComp, _nParType, _nBound, _strideBound, _boundOffset, _totalBound, nullptr);
	recorder.unitOperationStructure(_unitOpIdx, *this, expr);
}

unsigned int DeadEndFiltrationFlow::requiredADdirs() const CADET_NOEXCEPT
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
void DeadEndFiltrationFlow::prepareADvectors(const AdJacobianParams& adJac) const
{
...
void DeadEndFiltrationFlow::applyInitialCondition(const SimulationState& simState) const
{
	// Inlet DOFs
...
void DeadEndFiltrationFlow::readInitialCondition(IParameterProvider& paramProvider)
{
...
void DeadEndFiltrationFlow::consistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
...
void DeadEndFiltrationFlow::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY, double* const vecStateYdot, util::ThreadLocalStorage& threadLocalMem) 
{
...
void DeadEndFiltrationFlow::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
...
void DeadEndFiltrationFlow::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY, double* const vecStateYdot, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
	// It is assumed that the bound states are (approximately) correctly initialized.
	// Thus, only the liquid phase has to be initialized
...
void DeadEndFiltrationFlow::leanConsistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	consistentInitialSensitivity(simTime, simState, vecSensY, vecSensYdot, adRes, threadLocalMem);
}

int DeadEndFiltrationFlow::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
...
int DeadEndFiltrationFlow::residualImpl(double t, unsigned int secIdx, StateType const* const y, double const* const yDot, ResidualType* const res, LinearBufferAllocator tlmAlloc)
{
...
int DeadEndFiltrationFlow::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res,
...
int DeadEndFiltrationFlow::residualWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
	return residual(simTime, simState, res, adJac, threadLocalMem, true, false);
}

int DeadEndFiltrationFlow::residualSensFwdAdOnly(const SimulationTime& simTime, const ConstSimulationState& simState, active* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
...
int DeadEndFiltrationFlow::residualSensFwdWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
...
void DeadEndFiltrationFlow::initializeSensitivityStates(const std::vector<double*>& vecSensY) const
{
	const unsigned int nDof = numPureDofs();
...
void DeadEndFiltrationFlow::consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	// TODO: Handle v = 0
...
void DeadEndFiltrationFlow::multiplyWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* yS, double alpha, double beta, double* ret)
{
	const double flowIn = static_cast<double>(_flowRateIn);
...
void DeadEndFiltrationFlow::multiplyWithDerivativeJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* sDot, double* ret)
{
...
int DeadEndFiltrationFlow::linearSolve(double t, double alpha, double tol, double* const rhs, double const* const weight,
...
void DeadEndFiltrationFlow::addTimeDerivativeJacobian(double t, double alpha, const ConstSimulationState& simState, MatrixType& mat)
{
	double const* const c = simState.vecStateY + _nComp;
	double const* const q = simState.vecStateY + 2 * _nComp;
	const double v = simState.vecStateY[2 * _nComp + _totalBound];
 ...
 */
void DeadEndFiltrationFlow::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
 ...
 * @param [in] adDirOffset Number of AD directions used for non-Jacobian purposes (e.g., parameter sensitivities)
 */
void DeadEndFiltrationFlow::checkAnalyticJacobianAgainstAd(active const* const adRes, unsigned int adDirOffset) const
{
...
unsigned int DeadEndFiltrationFlow::Exporter::numSolidPhaseDofs() const CADET_NOEXCEPT
{
	return _totalBound;
}

int DeadEndFiltrationFlow::Exporter::writeMobilePhase(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}

int DeadEndFiltrationFlow::Exporter::writeSolidPhase(double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int DeadEndFiltrationFlow::Exporter::writeSolidPhase(unsigned int parType, double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int DeadEndFiltrationFlow::Exporter::writeVolume(double* buffer) const
{
	*buffer = _data[2 * _nComp + _totalBound];
	return 1;
}

int DeadEndFiltrationFlow::Exporter::writeInlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
...
int DeadEndFiltrationFlow::Exporter::writeInlet(double* buffer) const
{
	std::copy_n(_data, _nComp, buffer);
	return _nComp;
}

int DeadEndFiltrationFlow::Exporter::writeOutlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
...
int DeadEndFiltrationFlow::Exporter::writeOutlet(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}
...
{
	models[DeadEndFiltrationFlow::identifier()] = [](UnitOpIdx uoId) { return new DeadEndFiltrationFlow(uoId); };
}
