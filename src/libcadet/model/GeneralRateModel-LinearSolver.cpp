 ...
 */
int GeneralRateModel::linearSolve(double t, double alpha, double outerTol, double* const rhs,
...
// rhs is passed twice but due to the values in jacA the writes happen to a different area of the rhs than the reads.
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
		...
		{
			_jacFP[type * _disc.nCol + par].multiplySubtract(
	 ...
	 */
	void GeneralRateModel::assembleDiscretizedJacobianParticleBlock(unsigned int parType, unsigned int pblk,
	 ...
	 * @param [in] parType Index of the particle type
	 */
	void GeneralRateModel::addTimeDerivativeToJacobianParticleShell(linalg::FactorizableBandMatrix::RowIterator& jac,
