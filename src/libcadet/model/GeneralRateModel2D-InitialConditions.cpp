...
int GeneralRateModel2D::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection,
												   double adValue)
{
	...
	if (_singleBinding)
	{
		...
		{
			if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) &&
				(pId.boundState == BoundStateIndep) && (pId.particleType == ParTypeIndep) &&
				(pId.component != CompIndep) && (pId.reaction == ReactionIndep))
			{
				_sensParams.insert(&_initCp[pId.component]);
				for (unsigned int r = 0; r < _disc.nRad; ++r)
				{
					for (unsigned int t = 0; t < _disc.nParType; ++t)
						_initCp[r * _disc.nComp * _disc.nParType + t * _disc.nComp + pId.component].setADValue(
							adDirection, adValue);
				}
				return 1;
			}
		...
		else
		{
				...
				(pId.component != CompIndep) && (pId.reaction != ReactionIndep))
			{
				...
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					_initCp[pId.reaction * _disc.nComp * _disc.nParType + t * _disc.nComp + pId.component].setADValue(
						adDirection, adValue);
		...
		{
			if ((pId.name == hashString("INIT_Q")) && (pId.section == SectionIndep) &&
				(pId.boundState != BoundStateIndep) && (pId.particleType == ParTypeIndep) &&
				(pId.component != CompIndep) && (pId.reaction == ReactionIndep))
			{
				_sensParams.insert(
					&_initQ[_disc.nBoundBeforeType[0] + _disc.boundOffset[pId.component] + pId.boundState]);
				for (unsigned int r = 0; r < _disc.nRad; ++r)
				{
					for (unsigned int t = 0; t < _disc.nParType; ++t)
						_initQ[r * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[t] +
							   _disc.boundOffset[t * _disc.nComp + pId.component] + pId.boundState]
							.setADValue(adDirection, adValue);
				}
				return 1;
			}
		...
		else
		{
			...
			{
				_sensParams.insert(
					&_initQ[pId.reaction * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[0] +
							_disc.boundOffset[pId.component] + pId.boundState]);
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					_initQ[pId.reaction * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[t] +
						   _disc.boundOffset[t * _disc.nComp + pId.component] + pId.boundState]
						.setADValue(adDirection, adValue);
	...
	else
	{
		if (_singleRadiusInitCp)
		{
				...
				(pId.component != CompIndep) && (pId.reaction == ReactionIndep))
			{
				...
				for (unsigned int r = 0; r < _disc.nRad; ++r)
					_initCp[r * _disc.nComp * _disc.nParType + pId.particleType * _disc.nComp + pId.component]
						.setADValue(adDirection, adValue);
		...
		else
		{
			if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) &&
				...
				(pId.component != CompIndep) && (pId.reaction != ReactionIndep))
			{
				_sensParams.insert(&_initCp[pId.reaction * _disc.nComp * _disc.nParType +
											pId.particleType * _disc.nComp + pId.component]);
				_initCp[pId.reaction * _disc.nComp * _disc.nParType + pId.particleType * _disc.nComp + pId.component]
					.setADValue(adDirection, adValue);
		...
		if (_singleRadiusInitQ)
		{
			...
			{
				_sensParams.insert(
					&_initQ[_disc.nBoundBeforeType[pId.particleType] +
							_disc.boundOffset[pId.particleType * _disc.nComp + pId.component] + pId.boundState]);
				for (unsigned int r = 0; r < _disc.nRad; ++r)
					_initQ[r * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[pId.particleType] +
						   _disc.boundOffset[pId.particleType * _disc.nComp + pId.component] + pId.boundState]
						.setADValue(adDirection, adValue);
		...
		else
		{
			...
			{
				_sensParams.insert(
					&_initQ[pId.reaction * _disc.strideBound[_disc.nParType] +
							_disc.nBoundBeforeType[pId.particleType] +
							_disc.boundOffset[pId.particleType * _disc.nComp + pId.component] + pId.boundState]);
				_initQ[pId.reaction * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[pId.particleType] +
					   _disc.boundOffset[pId.particleType * _disc.nComp + pId.component] + pId.boundState]
					.setADValue(adDirection, adValue);
...
int GeneralRateModel2D::multiplexInitialConditions(const cadet::ParameterId& pId, double val, bool checkSens)
{
	...
	if (_singleBinding)
	{
		...
		else
		{
			...
			{
				if (checkSens &&
					!contains(_sensParams, &_initCp[pId.reaction * _disc.nComp * _disc.nParType + pId.component]))
					return -1;

				for (unsigned int t = 0; t < _disc.nParType; ++t)
					_initCp[pId.reaction * _disc.nComp * _disc.nParType + t * _disc.nComp + pId.component].setValue(
						val);
	...
	else
	{
		if (_singleRadiusInitCp)
		{
				...
				(pId.component != CompIndep) && (pId.reaction == ReactionIndep))
			{
				...
				for (unsigned int r = 0; r < _disc.nRad; ++r)
					_initCp[r * _disc.nComp * _disc.nParType + pId.particleType * _disc.nComp + pId.component].setValue(
						val);
		...
		else
		{
			if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) &&
				...
				(pId.component != CompIndep) && (pId.reaction != ReactionIndep))
			{
				if (checkSens && !contains(_sensParams, &_initCp[pId.reaction * _disc.nComp * _disc.nParType +
																 pId.particleType * _disc.nComp + pId.component]))
					return -1;
	...
	for (unsigned int col = 0; col < _disc.nCol; ++col)
	{
		...
		for (unsigned int rad = 0; rad < _disc.nRad; ++rad)
		{
			...
			for (unsigned comp = 0; comp < _disc.nComp; ++comp)
				stateYbulk[col * idxr.strideColAxialCell() + rad * idxr.strideColRadialCell() +
		...
		for (unsigned int col = 0; col < _disc.nCol * _disc.nRad; ++col)
		{
			...
			for (unsigned int shell = 0; shell < _disc.nParCell[type]; ++shell)
			{
				...
				for (unsigned int comp = 0; comp < _disc.nComp; ++comp)
					simState.vecStateY[shellOffset + comp] =
						static_cast<double>(_initCp[comp + _disc.nComp * type + rad * _disc.nComp * _disc.nParType]);

				// Initialize q
				active const* const iq =
					_initQ.data() + rad * _disc.strideBound[_disc.nParType] + _disc.nBoundBeforeType[type];
...
void GeneralRateModel2D::readInitialCondition(IParameterProvider& paramProvider)
{
	...
	if (((initC.size() < _disc.nComp) && _singleRadiusInitC) ||
		((initC.size() < _disc.nComp * _disc.nRad) && !_singleRadiusInitC))
	...
	if (paramProvider.exists("INIT_CP"))
	{
		...
		if (((initCp.size() < _disc.nComp * _disc.nParType) && !_singleBinding && _singleRadiusInitCp) ||
			((initCp.size() < _disc.nComp) && _singleBinding && _singleRadiusInitCp) ||
		...
		{
			for (unsigned int r = 0; r < _disc.nRad; ++r)
				ad::copyToAd(initCp.data(), _initCp.data() + r * _disc.nComp * _disc.nParType,
		...
		else if (_singleBinding && !_singleRadiusInitCp)
		{
			...
			{
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					ad::copyToAd(initCp.data() + r * _disc.nComp,
								 _initCp.data() + t * _disc.nComp + r * _disc.nComp * _disc.nParType, _disc.nComp);
			}
		}
		else if (_singleBinding && _singleRadiusInitCp)
		{
			...
			{
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					ad::copyToAd(initCp.data(), _initCp.data() + t * _disc.nComp + r * _disc.nComp * _disc.nParType,
								 _disc.nComp);
			}
		}
	}
	else
	{
		...
		if (!_singleRadiusInitCp)
		{
			...
			{
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					ad::copyToAd(initC.data() + r * _disc.nComp,
								 _initCp.data() + t * _disc.nComp + r * _disc.nComp * _disc.nParType, _disc.nComp);
			}
		}
		else
		{
			...
			{
				for (unsigned int t = 0; t < _disc.nParType; ++t)
					ad::copyToAd(initC.data(), _initCp.data() + t * _disc.nComp + r * _disc.nComp * _disc.nParType,
								 _disc.nComp);
			}
		}
	}
	...
	if (paramProvider.exists("INIT_Q"))
	{
		...
		_singleRadiusInitQ =
			(initQ.size() == _disc.strideBound[0]) || (initQ.size() == _disc.strideBound[_disc.nParType]);
	}
	...
	if ((_disc.strideBound[_disc.nParType] > 0) &&
		(((initQ.size() < _disc.strideBound[_disc.nParType]) && !_singleBinding && _singleRadiusInitQ) ||
		 ((initQ.size() < _disc.strideBound[0]) && _singleBinding && _singleRadiusInitQ) ||
	...
	{
		for (unsigned int r = 0; r < _disc.nRad; ++r)
			ad::copyToAd(initQ.data(), _initQ.data() + r * _disc.strideBound[_disc.nParType],
						 _disc.strideBound[_disc.nParType]);
	}
	else if (_singleBinding && !_singleRadiusInitQ)
	{
		for (unsigned int r = 0; r < _disc.nRad; ++r)
		{
			for (unsigned int t = 0; t < _disc.nParType; ++t)
				ad::copyToAd(initQ.data() + r * _disc.strideBound[0],
							 _initQ.data() + _disc.nBoundBeforeType[t] + r * _disc.strideBound[_disc.nParType],
							 _disc.strideBound[t]);
		}
	}
	else if (_singleBinding && _singleRadiusInitQ)
 ...
 */
void GeneralRateModel2D::consistentInitialState(const SimulationTime& simTime, double* const vecStateY,
												...
												util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
...
#endif
		{
			...
			linalg::DenseMatrixView fullJacobianMatrix(_jacPdisc[type * _disc.nCol * _disc.nRad + pblk].data(), nullptr,
													   mask.len, mask.len);
			...
			linalg::DenseMatrixView jacobianMatrix(
				jacobianMem, _jacPdisc[type * _disc.nCol * _disc.nRad + pblk].pivot(), probSize, probSize);
			const parts::cell::CellParameters cellResParams{
				...
				_binding[type],
				(_dynReaction[type] && (_dynReaction[type]->numReactionsCombined() > 0)) ? _dynReaction[type]
																						 : nullptr};

			// This loop cannot be run in parallel without creating a Jacobian matrix for each thread which would
			// increase memory usage
			const int localOffsetToParticle =
				idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{static_cast<unsigned int>(pblk)});
			for (std::size_t shell = 0; shell < static_cast<std::size_t>(_disc.nParCell[type]); ++shell)
			{
				...
				double* const qShell =
					vecStateY + localOffsetToParticle + localOffsetInParticle + idxr.strideParLiquid();
				active* const localAdRes =
					adJac.adRes ? adJac.adRes + localOffsetToParticle + localOffsetInParticle : nullptr;
				active* const localAdY =
					adJac.adY ? adJac.adY + localOffsetToParticle + localOffsetInParticle : nullptr;

				const ColumnPosition colPos{
					z, 0.0,
					static_cast<double>(_parCenterRadius[_disc.nParCellsBeforeType[type] + shell]) /
						static_cast<double>(_parRadius[type])};

				// Determine whether nonlinear solver is required
				if (!_binding[type]->preConsistentInitialState(simTime.t, simTime.secIdx, colPos, qShell,
															   qShell - idxr.strideParLiquid(), tlmAlloc))
					continue;
				...
				linalg::conservedMoietiesFromPartitionedMask(mask, _disc.nBound + type * _disc.nComp, _disc.nComp,
															 qShell - _disc.nComp, conservedQuants,
															 static_cast<double>(_parPorosity[type]), epsQ);
				...
				if (localAdY && localAdRes)
				{
					jacFunc = [&](double const* const x, linalg::detail::DenseMatrixBase& mat) {
						// Copy over state vector to AD state vector (without changing directional values to keep seed
						// vectors) and initialize residuals with zero (also resetting directional values)
						...
						const double diff = ad::compareDenseJacobianWithBandedAd(
							localAdRes - localOffsetInParticle, localOffsetInParticle, adJac.adDirOffset,
							_jacP[type * _disc.nCol * _disc.nRad].lowerBandwidth(),
							_jacP[type * _disc.nCol * _disc.nRad].lowerBandwidth(),
							_jacP[type * _disc.nCol * _disc.nRad].upperBandwidth(), fullJacobianMatrix);
				...
				_binding[type]->postConsistentInitialState(simTime.t, simTime.secIdx, colPos, qShell,
														   qShell - idxr.strideParLiquid(), tlmAlloc);
			}
		} CADET_PARFOR_END;
	}
 ...
 */
void GeneralRateModel2D::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY,
...
#endif
	{
		...
		for (unsigned int j = 0; j < _disc.nParCell[type]; ++j)
		{
			...
			linalg::BandMatrix::RowIterator jacSolidOrig =
				_jacP[pblk].row(j * static_cast<unsigned int>(idxr.strideParShell(type)) +
								static_cast<unsigned int>(idxr.strideParLiquid()));
			...
			double* const qShellDot = vecStateYdot + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}) +
									  static_cast<int>(j) * idxr.strideParShell(type) + idxr.strideParLiquid();

			// Obtain derivative of fluxes wrt. time
			std::fill_n(dFluxDt, _disc.strideBound[type], 0.0);
			if (_binding[type]->dependsOnTime())
			{
				_binding[type]->timeDerivativeQuasiStationaryFluxes(
					simTime.t, simTime.secIdx,
					ColumnPosition{z, r,
								   static_cast<double>(_parCenterRadius[_disc.nParCellsBeforeType[type] + j]) /
									   static_cast<double>(_parRadius[type])},
		...
		// Solve
		const bool result2 =
			fbm.solve(scaleFactors, vecStateYdot + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}));
		if (!result2)
		{
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void GeneralRateModel2D::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY,
													const AdJacobianParams& adJac, double errorTol,
													util::ThreadLocalStorage& threadLocalMem)
{
 ...
 */
void GeneralRateModel2D::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY,
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
			for (unsigned int col = 0; col < _disc.nCol * _disc.nRad; ++col)
			{
				...
				for (unsigned int shell = 0; shell < _disc.nParCell[type]; ++shell)
				{
					...
					for (unsigned int comp = 0; comp < _disc.nComp; ++comp)
						stateYparticle[comp] =
							_initCp[comp + type * _disc.nComp + rad * _disc.nComp * _disc.nParType].getADValue(param);

					// Initialize q
					for (unsigned int bnd = 0; bnd < _disc.strideBound[type]; ++bnd)
						stateYparticleSolid[bnd] =
							_initQ[bnd + _disc.nBoundBeforeType[type] + _disc.strideBound[_disc.nParType] * rad]
								.getADValue(param);
				}
			}
		}
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModel2D::consistentInitialSensitivity(const SimulationTime& simTime,
													  const ConstSimulationState& simState,
													  std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot,
													  active const* const adRes,
													  util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
	...
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
			...
			{
				// Reuse memory of band matrix for dense matrix
				linalg::DenseMatrixView jacobianMatrix(_jacPdisc[type * _disc.nCol * _disc.nRad + pblk].data(),
													   _jacPdisc[type * _disc.nCol * _disc.nRad + pblk].pivot(),
													   probSize, probSize);
				...
				double* const maskedMultiplier =
					_tempState + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{static_cast<unsigned int>(pblk)});
				double* const scaleFactors =
					_tempState + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{static_cast<unsigned int>(pblk)});

				for (unsigned int shell = 0; shell < _disc.nParCell[type]; ++shell)
				{
					const int jacRowOffset = static_cast<int>(shell) * idxr.strideParShell(type) + _disc.nComp;
					const int localQOffset =
						idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{static_cast<unsigned int>(pblk)}) +
						static_cast<int>(shell) * idxr.strideParShell(type) + idxr.strideParLiquid();

					// Extract subproblem Jacobian from full Jacobian
					jacobianMatrix.setAll(0.0);
					linalg::copyMatrixSubset(_jacP[type * _disc.nCol * _disc.nRad + pblk], mask, mask, jacRowOffset, 0,
											 jacobianMatrix);
					...
					std::copy_n(sensY + localQOffset - idxr.strideParLiquid(), idxr.strideParShell(type),
								maskedMultiplier);
					linalg::fillVectorSubset(maskedMultiplier + _disc.nComp, mask, 0.0);

					// Assemble right hand side
					_jacP[type * _disc.nCol * _disc.nRad + pblk].submatrixMultiplyVector(
						maskedMultiplier, jacRowOffset, -static_cast<int>(_disc.nComp), _disc.strideBound[type],
						idxr.strideParShell(type), rhsUnmasked);
...
#endif
		{
			...
			for (unsigned int j = 0; j < _disc.nParCell[type]; ++j)
			{
				...
				if (_binding[type]->hasQuasiStationaryReactions())
				{
					...
					int const* const mask = _binding[type]->reactionQuasiStationarity();
					double* const qShellDot = sensYdot + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}) +
											  static_cast<int>(j) * idxr.strideParShell(type) + idxr.strideParLiquid();
			...
			// Solve
			const bool result2 =
				fbm.solve(scaleFactors, sensYdot + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}));
			if (!result2)
			{
				LOG(Error) << "Solve() failed for par block " << pblk << " (type " << type << " col " << par << ")";
			}
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModel2D::leanConsistentInitialSensitivity(const SimulationTime& simTime,
														  const ConstSimulationState& simState,
														  std::vector<double*>& vecSensY,
														  std::vector<double*>& vecSensYdot, active const* const adRes,
														  util::ThreadLocalStorage& threadLocalMem)
{
	if (isSectionDependent(_parDiffusionMode) || isSectionDependent(_parSurfDiffusionMode))
		LOG(Warning)
...
void GeneralRateModel2D::solveForFluxes(double* const vecState, const Indexer& idxr) const
{
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
		...
		for (unsigned int pblk = 0; pblk < _disc.nCol * _disc.nRad; ++pblk)
			jacFPtype[pblk].multiplySubtract(vecState + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{pblk}),
											 jf);
	}
}
