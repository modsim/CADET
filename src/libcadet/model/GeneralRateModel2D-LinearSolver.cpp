 ...
 *
 *          Exploiting the decomposition, the solution procedure @f$ x = J^{-1}b = \left( LU \right)^{-1}b = U^{-1}
 L^{-1} b @f$
 *          works as follows:
 *              -# Factorize the diagonal blocks @f$ J_0, \dots, J_{N_z} @f$
 *              -# Solve @f$ y = L^{-1} b @f$ by forward substitution. This is accomplished by first solving the
 diagonal
 *                 blocks independently, that is,
 *                 @f[ y_i = J_{i}^{-1} b_i. @f]
 *                 Then, calculate the flux-part @f$ y_f @f$ by substituting in the already calculated solutions @f$ y_i
 @f$:
 ...
 */
int GeneralRateModel2D::linearSolve(double t, double alpha, double outerTol, double* const rhs,
...
// rhs is passed twice but due to the values in jacA the writes happen to a different area of the rhs than the reads.
	...
	{
	const bool result =
		_convDispOp.solveDiscretizedJacobian(rhs + idxr.offsetC(), weight + idxr.offsetC(), nullptr, outerTol);
	if (cadet_unlikely(!result))
	{
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
		...
		{
			_jacFP[type * _disc.nCol * _disc.nRad + par].multiplySubtract(
				rhs + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}), rhs + idxr.offsetJf());
		}
	}
	...
	const bool result = _convDispOp.solveDiscretizedJacobian(localCol, weight + idxr.offsetC(), nullptr, outerTol);
	if (cadet_unlikely(!result))
	{
		LOG(Error) << "Solve() failed for bulk block";
	}
	 ...
	 *              -# Compute @f$ J_{f,i} \, J_i^{-1} \, J_{i,f} @f$ independently (in parallel with respect to index
	 @f$ i @f$)
	...
	{
		// Apply J_0^{-1}
		const bool result = _convDispOp.solveDiscretizedJacobian(_tempState + idxr.offsetC(), nullptr, nullptr, 0.0);
		if (cadet_unlikely(!result))
		{
			LOG(Error) << "Solve() failed for bulk block";
		}
...
#endif
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
			for (unsigned int par = 0; par < _disc.nCol * _disc.nRad; ++par)
			{
				...
				_jacFP[type * _disc.nCol * _disc.nRad + par].multiplySubtract(
					_tempState + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}), z);
			}
		}
	} CADET_PARNODE_END;
	 ...
	 * x = b \f] has to be solved. The system Jacobian of the original equations, \f[ \frac{\partial F}{\partial y}, \f]
	 *          is already computed (by AD or manually in residualImpl() with @c wantJac = true). This function is
	 * responsible for adding \f[ \alpha \frac{\partial F}{\partial \dot{y}} \f] to the system Jacobian, which yields
	 * the Jacobian of the time-discretized equations \f[ F\left(t, y_0, \sum_{k=0}^N \alpha_k y_k \right) = 0 \f] when
	 * a BDF method is used. The time integrator needs to solve this equation for @f$ y_0 @f$, which requires the
	 * solution of the linear system mentioned above (@f$ \alpha_0 = \alpha @f$ given in @p alpha).
	 *
	 ...
	 */
	void GeneralRateModel2D::assembleDiscretizedJacobianParticleBlock(unsigned int parType, unsigned int pblk,
																	  double alpha, const Indexer& idxr)
	{
	 ...
	 * @param [in] parType Index of the particle type
	 */
	void GeneralRateModel2D::addTimeDerivativeToJacobianParticleShell(linalg::FactorizableBandMatrix::RowIterator& jac,
																	  const Indexer& idxr, double alpha,
																	  unsigned int parType)
	{
	parts::cell::addTimeDerivativeToJacobianParticleShell<linalg::FactorizableBandMatrix::RowIterator, true>(
		jac, alpha, static_cast<double>(_parPorosity[parType]), _disc.nComp, _disc.nBound + _disc.nComp * parType,
		_poreAccessFactor.data() + _disc.nComp * parType, _disc.strideBound[parType],
		_disc.boundOffset + _disc.nComp * parType, _binding[parType]->reactionQuasiStationarity());
	}
