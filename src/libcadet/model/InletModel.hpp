...
protected:
	...
	int residualImpl(double t, unsigned int secIdx, const ConstSimulationState& simState, ResidualType* const res,
					 util::ThreadLocalStorage& threadLocalMem);
	...
	double* _inletDerivatives;    //!< Points to memory used for retrieving the derivatives of the inlet profile (do not
								  //!< delete since the memory is owned by _inletConcentrationsRaw)
	active* _inletConcentrations; //!< Provides memory for the inlet concentrations

	std::unordered_map<ParameterId, std::tuple<unsigned int, double>>
		_sensParamsInlet; //!< Maps an inlet parameter to its AD direction and derivative value
