...
int LumpedRateModelWithPores::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection,
														 double adValue)
{
	if (_singleBinding)
	{
 ...
 */
void LumpedRateModelWithPores::consistentInitialState(const SimulationTime& simTime, double* const vecStateY,
													  ...
													  util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
...
#endif
		{
			...
			linalg::DenseMatrixView fullJacobianMatrix(_jacPdisc[type].data() + pblk * mask.len * mask.len, nullptr,
													   mask.len, mask.len);
			...
			linalg::DenseMatrixView jacobianMatrix(static_cast<double*>(jacobianMemBuffer),
												   _jacPdisc[type].pivot() + pblk * probSize, probSize, probSize);
			...
			if (localAdY && localAdRes)
			{
				jacFunc = [&](double const* const x, linalg::detail::DenseMatrixBase& mat) {
					...
					const double diff = ad::compareDenseJacobianWithBandedAd(
						adJac.adRes + idxr.offsetCp(ParticleTypeIndex{type}), pblk * idxr.strideParBlock(type),
						adJac.adDirOffset, _jacP[type].lowerBandwidth(), _jacP[type].lowerBandwidth(),
						_jacP[type].upperBandwidth(), fullJacobianMatrix);
					LOG(Debug) << "MaxDiff: " << diff;
#endif

					// Extract Jacobian from AD
					ad::extractDenseJacobianFromBandedAd(adJac.adRes + idxr.offsetCp(ParticleTypeIndex{type}),
														 pblk * idxr.strideParBlock(type), adJac.adDirOffset,
														 _jacP[type].lowerBandwidth(), _jacP[type].lowerBandwidth(),
														 _jacP[type].upperBandwidth(), fullJacobianMatrix);
 ...
 */
void LumpedRateModelWithPores::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY,
...
#endif
	{
		...
		for (unsigned int pblk = 0; pblk < _disc.nCol; ++pblk)
		{
			...
			linalg::BandMatrix::RowIterator jacSolidOrig =
				_jacP[type].row(idxr.strideParBlock(type) * pblk + static_cast<unsigned int>(idxr.strideParLiquid()));
			...
			double* const qShellDot =
				vecStateYdot + idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}, ParticleIndex{pblk}) +
				idxr.strideParLiquid();
			...
			if (_binding[type]->dependsOnTime())
			{
				_binding[type]->timeDerivativeQuasiStationaryFluxes(
					simTime.t, simTime.secIdx, ColumnPosition{z, 0.0, static_cast<double>(_parRadius[type]) * 0.5},
		...
		const bool result2 = _jacPdisc[type].solve(
			scaleFactors, vecStateYdot + idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}));
		if (!result2)
		{
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void LumpedRateModelWithPores::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY,
														  const AdJacobianParams& adJac, double errorTol,
														  util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
 ...
 */
void LumpedRateModelWithPores::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY,
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithPores::consistentInitialSensitivity(
	...
	std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
			...
			{
				// Reuse memory of band matrix for dense matrix
				linalg::DenseMatrixView jacobianMatrix(_jacPdisc[type].data() + pblk * probSize * probSize,
													   _jacPdisc[type].pivot() + pblk * probSize, probSize, probSize);
				...
				const int jacRowOffset = idxr.strideParBlock(type) * pblk + _disc.nComp;
				...
				linalg::copyMatrixSubset(_jacP[type], mask, mask, jacRowOffset, 0, jacobianMatrix);

				// Construct right hand side
				linalg::selectVectorSubset(sensYdot + localQOffset, mask, rhs);

				// Zero out masked elements
				std::copy_n(sensY + localQOffset - idxr.strideParLiquid(), _disc.nComp + _disc.strideBound[type],
							maskedMultiplier);
				linalg::fillVectorSubset(maskedMultiplier + _disc.nComp, mask, 0.0);

				// Assemble right hand side
				_jacP[type].submatrixMultiplyVector(maskedMultiplier, jacRowOffset, -static_cast<int>(_disc.nComp),
													_disc.strideBound[type], _disc.nComp + _disc.strideBound[type],
													rhsUnmasked);
...
#endif
		{
			...
			for (unsigned int pblk = 0; pblk < _disc.nCol; ++pblk)
			{
				...
				{
					// Get iterators to beginning of solid phase
					linalg::BandMatrix::RowIterator jacSolidOrig = _jacP[type].row(
						idxr.strideParBlock(type) * pblk + static_cast<unsigned int>(idxr.strideParLiquid()));
					linalg::FactorizableBandMatrix::RowIterator jacSolid = jac - idxr.strideParBound(type);

					int const* const mask = _binding[type]->reactionQuasiStationarity();
					double* const qShellDot =
						sensYdot +
						idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}, ParticleIndex{pblk}) +
						idxr.strideParLiquid();
			...
			// Solve
			const bool result2 = _jacPdisc[type].solve(
				scaleFactors, sensYdot + idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}));
			if (!result2)
			{
				LOG(Error) << "Solve() failed for par type block " << type;
			}
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithPores::leanConsistentInitialSensitivity(
	const SimulationTime& simTime, const ConstSimulationState& simState, std::vector<double*>& vecSensY,
	std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
