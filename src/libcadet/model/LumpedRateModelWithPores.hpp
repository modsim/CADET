...
c_i}{\partial z^2} - \frac{1 - \varepsilon_c}{\varepsilon_c} \frac{3 k_{f,i}}{r_p} j_{f,i} \\
	\frac{\partial c_{p,i}}{\partial t} + \frac{1 - \varepsilon_p}{\varepsilon_p} \frac{\partial q_{i}}{\partial t} &=
\frac{3 k_{f,i}}{\varepsilon_p r_p} j_{f,i} \\ a \frac{\partial q_i}{\partial t} &= f_{\text{iso}}(c_p, q) \end{align}
@f]
@f[ \begin{align}
	j_{f,i} = c_i - c_{p,i}
\end{align} @f]
...
public:
	...
	{
		return "LUMPED_RATE_MODEL_WITH_PORES";
	}
...
protected:
	...
	template <typename StateType, typename ResidualType, typename ParamType, bool wantJac>
	int residualBulk(double t, unsigned int secIdx, StateType const* yBase, double const* yDotBase,
	...
	void addTimeDerivativeToJacobianParticleBlock(linalg::FactorizableBandMatrix::RowIterator& jac, const Indexer& idxr,
	...
	std::vector<linalg::FactorizableBandMatrix> _jacPdisc; //!< Particle jacobian diagonal blocks (all of them for each
														   //!< particle type) with time derivatives from BDF method
	...
	std::vector<linalg::DoubleSparseMatrix>
		_jacPF; //!< Jacobian blocks connecting particle states and fluxes (particle transport boundary condition)
	std::vector<linalg::DoubleSparseMatrix>
		_jacFP; //!< Jacobian blocks connecting fluxes and particle states (flux equation)
	...
	{
	public:
		Exporter(const Discretization& disc, const LumpedRateModelWithPores& model, double const* data)
		...
		{
			return _disc.strideBound[parType] * _disc.nCol;
		}
