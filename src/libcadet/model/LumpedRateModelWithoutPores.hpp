 ...
 *
 * @f[\begin{align}
	\frac{\partial c_i}{\partial t} + \frac{1 - \varepsilon_t}{\varepsilon_t} \frac{\partial q_{i}}{\partial t} &= - u
\frac{\partial c_i}{\partial z} + D_{\text{ax},i} \frac{\partial^2 c_i}{\partial z^2} \\ a \frac{\partial q_i}{\partial
t} &= f_{\text{iso}}(c, q) \end{align} @f]
...
public:
	...
	{
		return "LUMPED_RATE_MODEL_WITHOUT_PORES";
	}
	...
	virtual std::vector<double> benchmarkTimings() const
	{
		return std::vector<double>({static_cast<double>(numDofs()), _timerResidual.totalElapsedTime(),
									...
									_timerResidualSensPar.totalElapsedTime(), _timerConsistentInit.totalElapsedTime(),
									_timerConsistentInitPar.totalElapsedTime(), _timerLinearSolve.totalElapsedTime()});
	}
	...
	virtual char const* const* benchmarkDescriptions() const
	{
		static const char* const desc[] = {
			...
			"ConsistentInit", "ConsistentInitPar", "LinearSolve"};
		return desc;
	}
#endif
...
protected:
	...
	void addTimeDerivativeToJacobianCell(linalg::FactorizableBandMatrix::RowIterator& jac, const Indexer& idxr,
										 double alpha, double invBetaP) const;
	...
	public:
		...
		{
			return static_cast<int>(_disc.nComp + _disc.strideBound);
		}
		...
		inline int strideColLiquid() const CADET_NOEXCEPT
		{
			return static_cast<int>(_disc.nComp);
		}
		inline int strideColBound() const CADET_NOEXCEPT
		{
			return static_cast<int>(_disc.strideBound);
		}
		...
		inline int offsetBoundComp(unsigned int comp) const CADET_NOEXCEPT
		{
			return _disc.boundOffset[comp];
		}
		...
		{
			return data + offsetC() + strideColLiquid();
		}
	...
	{
	public:
		Exporter(const Discretization& disc, const LumpedRateModelWithoutPores& model, double const* data)
		...
		{
			return _disc.strideBound * _disc.nCol;
		}
		...
		virtual double const* concentration() const
		{
			return _idx.c(_data);
		}
		virtual double const* flux() const
		{
			return nullptr;
		}
		virtual double const* particleMobilePhase(unsigned int parType) const
		{
			return nullptr;
		}
		virtual double const* solidPhase(unsigned int parType) const
		{
			return _idx.q(_data);
		}
		virtual double const* volume() const
		{
			return nullptr;
		}
