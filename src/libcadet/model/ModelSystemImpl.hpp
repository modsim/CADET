...
public:
	...
	virtual int residualSensFwd(unsigned int nSens, const SimulationTime& simTime, const ConstSimulationState& simState,
								double const* const res, const std::vector<const double*>& yS,
	...
	virtual void consistentInitialConditions(const SimulationTime& simTime, const SimulationState& simState,
	...
	virtual void leanConsistentInitialConditions(const SimulationTime& simTime, const SimulationState& simState,
												 const AdJacobianParams& adJac, double errorTol);
	...
	virtual std::vector<double> calculateErrorTolsForAdditionalDofs(double const* errorTol,
	...
	virtual std::vector<double> benchmarkTimings() const
	{
		return std::vector<double>({static_cast<double>(numDofs()), _timerResidual.totalElapsedTime(),
									...
									_timerLinearAssemble.totalElapsedTime(), _timerLinearSolve.totalElapsedTime(),
									_timerMatVec.totalElapsedTime(), static_cast<double>(_gmres.numIterations())});
	}

	virtual char const* const* benchmarkDescriptions() const
	{
		static const char* const desc[] = {"DOFs",           "Residual",    "ResidualSens", "ConsistentInit",
										   "LinearAssemble", "LinearSolve", "MatVec",       "NumGMRESIter"};
		return desc;
	}
#endif

	void multiplyWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* yS,
							  double alpha, double beta, double* ret);
	void multiplyWithDerivativeJacobian(const SimulationTime& simTime, const ConstSimulationState& simState,
										double const* yS, double* ret);

#ifdef CADET_DEBUG
	void genJacobian(const SimulationTime& simTime, const ConstSimulationState& simState);

	void genJacobian(unsigned int nSens, const SimulationTime& simTime, const ConstSimulationState& simState,
					 double const* const res, const std::vector<const double*>& yS,
					 const std::vector<const double*>& ySdot, const std::vector<double*>& resS, active* const adRes,
...
protected:
	...
	int schurComplementMatrixVector(double const* x, double* z, double t, double alpha, double outerTol,
									double const* const weight, const ConstSimulationState& simState) const;

	void configureSwitches(IParameterProvider& paramProvider);

	template <typename StateType, typename ResidualType, typename ParamType>
	void residualConnectUnitOps(unsigned int secIdx, StateType const* const y, double const* const yDot,
	...
	int dResDpFwdWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState,
							  const AdJacobianParams& adJac);
	...
	template <typename ParamType> bool setParameterImpl(const ParameterId& pId, const ParamType value);

	void checkConnectionList(const std::vector<double>& conn, std::vector<int>& connOnly, std::vector<double>& flow,
							 std::vector<double>& flowLin, std::vector<double>& flowQuad, std::vector<double>& flowCub,
							 unsigned int idxSwitch) const;

	template <typename tag_t>
	void consistentInitialConditionAlgorithm(const SimulationTime& simTime, const SimulationState& simState,
											 const AdJacobianParams& adJac, double errorTol);
	 ...
	 */
	inline unsigned int numCouplingDOF() const CADET_NOEXCEPT
	{
		return _couplingIdxMap.size();
	}
	...
	util::SlicedVector<active>
		_flowRatesLin; //!< Vector of linear coefficients of connection flow rates for each section
	util::SlicedVector<active>
		_flowRatesQuad; //!< Vector of quadratic coefficients of connection flow rates for each section
	util::SlicedVector<active>
		_flowRatesCub; //!< Vector of cubic coefficients of connection flow rates for each section
	...
	util::SlicedVector<int> _linearModelOrdering;  //!< Dependency-consistent ordering of unit operation models for
												   //!< linear execution (for each switch)
	...
	util::SlicedVector<active>
		_totalInletFlowLin; //!< Total linear flow rate coefficient into each inlet at the current section
	util::SlicedVector<active>
		_totalInletFlowQuad; //!< Total quadratic flow rate coefficient into each inlet at the current section
	util::SlicedVector<active>
		_totalInletFlowCub; //!< Total cubic flow rate coefficient into each inlet at the current section
	util::SlicedVector<active> _totalOutletFlow; //!< Total flow rate into each outlet at the current section
	util::SlicedVector<active>
		_totalOutletFlowLin; //!< Total linear flow rate coefficient into each outlet at the current section
	util::SlicedVector<active>
		_totalOutletFlowQuad; //!< Total quadratic flow rate coefficient into each outlet at the current section
	util::SlicedVector<active>
		_totalOutletFlowCub; //!< Total cubic flow rate coefficient into each outlet at the current section
	...
	std::map<std::tuple<unsigned int, unsigned int, unsigned int>, unsigned int>
		_couplingIdxMap; //!< Maps (UnitOpIdx, PortIdx, CompIdx) to local coupling DOF index
