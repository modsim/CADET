...
/**
 * @brief Defines a parameter dependence relation
...
{
public:
	virtual ~IParameterDependence() CADET_NOEXCEPT
	{
	}
	 ...
	 * @details After construction of an IParameterDependence object, configureModelDiscretization() is called.
	 ...
	 */
	virtual active liquidParameter(const ColumnPosition& colPos, const active& param, active const* y,
								   int comp) const = 0;
	virtual active liquidParameter(const ColumnPosition& colPos, const active& param, double const* y,
								   int comp) const = 0;
	 ...
	 * @param [in,out] jac Row iterator pointing to the row of component @p comp in the underlying matrix that stores
	 * the Jacobian
	 */
	virtual void analyticJacobianLiquidAdd(const ColumnPosition& colPos, double param, double const* y, int comp,
										   double factor, int offset, linalg::BandMatrix::RowIterator jac) const = 0;
	virtual void analyticJacobianLiquidAdd(const ColumnPosition& colPos, double param, double const* y, int comp,
										   double factor, int offset, linalg::DenseBandedRowIterator jac) const = 0;
	 ...
	 */
	virtual void analyticJacobianLiquidAdd(const ColumnPosition& colPos, double param, double const* y, int comp,
										   double factor, int offset, int row,
										   linalg::DoubleSparseMatrix& jac) const = 0;
	 ...
	 * @return Actual parameter value
	 */
	virtual active combinedParameterLiquid(const ColumnPosition& colPos, const active& param, active const* yLiquid,
										   active const* ySolid, int comp) const = 0;
	virtual active combinedParameterLiquid(const ColumnPosition& colPos, const active& param, double const* yLiquid,
										   double const* ySolid, int comp) const = 0;
	virtual active combinedParameterLiquid(const ColumnPosition& colPos, double param, active const* yLiquid,
										   active const* ySolid, int comp) const = 0;
	virtual double combinedParameterLiquid(const ColumnPosition& colPos, double param, double const* yLiquid,
										   double const* ySolid, int comp) const = 0;
	 ...
	 * @return Actual parameter value
	 */
	virtual active combinedParameterSolid(const ColumnPosition& colPos, const active& param, active const* yLiquid,
										  active const* ySolid, int bnd) const = 0;
	virtual active combinedParameterSolid(const ColumnPosition& colPos, const active& param, double const* yLiquid,
										  double const* ySolid, int bnd) const = 0;
	virtual active combinedParameterSolid(const ColumnPosition& colPos, double param, active const* yLiquid,
										  active const* ySolid, int bnd) const = 0;
	virtual double combinedParameterSolid(const ColumnPosition& colPos, double param, double const* yLiquid,
										  double const* ySolid, int bnd) const = 0;
	...
	virtual void analyticJacobianCombinedAddLiquid(const ColumnPosition& colPos, double param, double const* yLiquid,
												   ...
												   linalg::BandMatrix::RowIterator jac) const = 0;
	virtual void analyticJacobianCombinedAddLiquid(const ColumnPosition& colPos, double param, double const* yLiquid,
												   double const* ySolid, int comp, double factor, int offset,
												   linalg::DenseBandedRowIterator jac) const = 0;
	 ...
	 * @param [in] factor Factor @f$ \gamma @f$
	 * @param [in] offset Diaognal offset in the row iterator
	 * @param [in,out] jac Row iterator pointing to the row of bound state @p bnd in the underlying matrix that stores
	 * the Jacobian
	 */
