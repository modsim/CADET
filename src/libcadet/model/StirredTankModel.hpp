 ...
 * @f[\begin{align}
	\frac{\mathrm{d}}{\mathrm{d} t}\left( V \left[ c_i + \frac{1}{\beta} \sum_{j=1}^{N_{\text{bnd},i}} q_{i,j} \right]
\right) &= F_{\text{in}} c_{\text{in},i} - F_{\text{out}} c_i \\
...
public:
	...
	inline const std::vector<active>& flowRateFilter() const
	{
		return _flowRateFilter;
	}
	inline std::vector<active>& flowRateFilter()
	{
		return _flowRateFilter;
	}
	inline void flowRateFilter(const std::vector<active>& frf)
	{
		_flowRateFilter = frf;
	}
	inline void setFlowRates(double in, double out) CADET_NOEXCEPT
	{
		_flowRateIn = in;
		_flowRateOut = out;
	}
...
protected:
	...
	int residualImpl(double t, unsigned int secIdx, StateType const* const y, double const* const yDot,
					 ResidualType* const res, LinearBufferAllocator threadLocalMem);
	...
	unsigned int* _boundOffset; //!< Array with offset to the first bound state of each component in the solid phase for
								//!< each particle type
	...
	std::vector<active> _initConditions;    //!< Initial conditions, ordering: Liquid phase concentration, solid phase
	...
	{
	public:
		Exporter(unsigned int nComp, unsigned int nParType, unsigned int const* nBound, unsigned int const* strideBound,
				 unsigned int const* boundOffset, unsigned int totalBound, double const* data)
			: _data(data), _nComp(nComp), _nParType(nParType), _nBound(nBound), _strideBound(strideBound),
			  _boundOffset(boundOffset), _totalBound(totalBound)
		{
		}
