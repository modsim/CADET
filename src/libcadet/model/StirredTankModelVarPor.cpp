...
#include "model/StirredTankModelVarPor.hpp"
...
CSTRModelVarPor::~CSTRModelVarPor() CADET_NOEXCEPT
{
	delete[] _boundOffset;
	delete[] _nBound;
	delete[] _strideBound;
...
bool CSTRModelVarPor::usesAD() const CADET_NOEXCEPT
{
...
void CSTRModelVarPor::setFlowRates(active const* in, active const* out) CADET_NOEXCEPT
{ 
	_flowRateIn = in[0];
	_flowRateOut = out[0];
}

bool CSTRModelVarPor::configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper)
{
...
bool CSTRModelVarPor::configure(IParameterProvider& paramProvider)
{
...
unsigned int CSTRModelVarPor::threadLocalMemorySize() const CADET_NOEXCEPT
{
...
void CSTRModelVarPor::setSectionTimes(double const* secTimes, bool const* secContinuity, unsigned int nSections)
{
}

void CSTRModelVarPor::useAnalyticJacobian(const bool analyticJac)
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
void CSTRModelVarPor::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx, const ConstSimulationState& simState, const AdJacobianParams& adJac)
{
...
void CSTRModelVarPor::reportSolution(ISolutionRecorder& recorder, double const* const solution) const
{
...
void CSTRModelVarPor::reportSolutionStructure(ISolutionRecorder& recorder) const
{
	Exporter expr(_nComp, _nParType, _nBound, _strideBound, _boundOffset, _totalBound, nullptr);
	recorder.unitOperationStructure(_unitOpIdx, *this, expr);
}

unsigned int CSTRModelVarPor::requiredADdirs() const CADET_NOEXCEPT
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
	if (_analyticJac)
...
void CSTRModelVarPor::prepareADvectors(const AdJacobianParams& adJac) const
{
...
void CSTRModelVarPor::applyInitialCondition(const SimulationState& simState) const
{
	// Inlet DOFs
...
void CSTRModelVarPor::readInitialCondition(IParameterProvider& paramProvider)
{
...
void CSTRModelVarPor::consistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
	...
	if (v == 0.0)
	{
		...
		const double qFactor = vDot * (1.0 / static_cast<double>(porosity) - 1.0) / denom;
		for (unsigned int type = 0; type < _nParType; ++type)
		{
...
void CSTRModelVarPor::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY, double* const vecStateYdot, util::ThreadLocalStorage& threadLocalMem)
{
	...
	if (v == 0.0)
	{
		...
		else
		{
			...
			//AB const double invBeta = 1.0 / porosity - 1.0;
			const double vsolid = static_cast<double>(_constSolidVolume);
			for (unsigned int i = 0; i < _nComp; ++i, ++itRow)
			{
				itRow.setAll(0.0);
...
void CSTRModelVarPor::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
...
void CSTRModelVarPor::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY, double* const vecStateYdot, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
	// It is assumed that the bound states are (approximately) correctly initialized.
	// Thus, only the liquid phase has to be initialized
...
void CSTRModelVarPor::leanConsistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	consistentInitialSensitivity(simTime, simState, vecSensY, vecSensYdot, adRes, threadLocalMem);
}

int CSTRModelVarPor::jacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
	return residual(simTime, simState, res, adJac, threadLocalMem, true, false);
}

int CSTRModelVarPor::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
...
int CSTRModelVarPor::residualImpl(double t, unsigned int secIdx, StateType const* const y, double const* const yDot, ResidualType* const res, LinearBufferAllocator tlmAlloc)
{
	...
	//AB Concentrations: \dot{V} * c + V * \dot{c} + vsoild * sum_j sum_m \dot{q}_{j,m}] = c_in * F_in - c * F_out Q
	//AB const ParamType invBeta = 1.0 / static_cast<ParamType>(_porosity) - 1.0;
	...
	if (wantJac)
	{
		...
		for (unsigned int i = 0; i < _nComp; ++i)
		{
			_jac.native(i, i) = vDotTimeFactor + static_cast<double>(flowOut); //AB dF/dci = v_liquidDot - F_out ? 

			if (cadet_likely(yDot))
			{
				double qDotSum = 0.0;
				//AB const double vDotInvBeta = vDotTimeFactor * static_cast<double>(invBeta);
				for (unsigned int type = 0; type < _nParType; ++type)
				{
					...
					const double vsolidParVolFrac = static_cast<double>(vsolid) * static_cast<double>(_parTypeVolFrac[type]); 
					double qDotSumType = 0.0;
					for (unsigned int j = 0; j < _nBound[type * _nComp + i]; ++j)
					{
	...
	if (_dynReactionBulk && (_dynReactionBulk->numReactionsLiquid() > 0))
	{
		...
		{
			for (unsigned int comp = 0; comp < _nComp; ++comp)
				_jac.native(comp, _nComp + _totalBound) += static_cast<double>(flux[comp]); //AB dF/dvliquid = flux
	...
	for (unsigned int type = 0; type < _nParType; ++type)
	{
		...
		if (dynReaction && (dynReaction->numReactionsCombined() > 0))
		{
			...
			if (wantJac)
			{
				...
				//AB kann das nicht weg? dF_{reac}/dV = 0
				// dRes / dV
				//idx = 0;
				//for (unsigned int comp = 0; comp < _nComp; ++comp)
				//{
...
int CSTRModelVarPor::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res,
...
int CSTRModelVarPor::residualWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
	return residual(simTime, simState, res, adJac, threadLocalMem, true, false);
}

int CSTRModelVarPor::residualSensFwdAdOnly(const SimulationTime& simTime, const ConstSimulationState& simState, active* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
...
int CSTRModelVarPor::residualSensFwdWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
...
void CSTRModelVarPor::initializeSensitivityStates(const std::vector<double*>& vecSensY) const
{
	const unsigned int nDof = numPureDofs();
...
void CSTRModelVarPor::consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	// TODO: Handle v = 0
...
void CSTRModelVarPor::multiplyWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* yS, double alpha, double beta, double* ret)
{
	const double flowIn = static_cast<double>(_flowRateIn);
...
void CSTRModelVarPor::multiplyWithDerivativeJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* sDot, double* ret)
{
	...
	for (unsigned int i = 0; i < _nComp; ++i)
	{
		...
		for (unsigned int type = 0; type < _nParType; ++type)
		{
			...
			const unsigned int localOffset = _nComp + _offsetParType[type] + _boundOffset[type * _nComp + i];
			//AB const double vInvBetaParVolFrac = vInvBeta * static_cast<double>(_parTypeVolFrac[type]);
			const double vSolidParVolFrac = vSolid * static_cast<double>(_parTypeVolFrac[type]);
			double qSumType = 0.0;
			for (unsigned int j = 0; j < _nBound[type * _nComp + i]; ++j)
			{
				//AB r[i] += vInvBetaParVolFrac * s[localOffset + j];
				r[i] += vSolidParVolFrac * s[localOffset + j]; //AB dRes/d_qDot
				// + _nComp: Moves over liquid phase components
				// + _offsetParType[type]: Moves to particle type
				// + _boundOffset[i]: Moves over bound states of previous components
				// + j: Moves to current bound state j of component i
		...
		//AB r[i] += (c[i] + invBeta * qSum) * s[_nComp + _totalBound];
		r[i] += c[i] * s[_nComp + _totalBound]; //dRes/dvSolidDot
	}
...
int CSTRModelVarPor::linearSolve(double t, double alpha, double tol, double* const rhs, double const* const weight,
...
void CSTRModelVarPor::addTimeDerivativeJacobian(double t, double alpha, const ConstSimulationState& simState, MatrixType& mat)
{	
	double const* const c = simState.vecStateY + _nComp;
	double const* const q = simState.vecStateY + 2 * _nComp;
	const double v = simState.vecStateY[2 * _nComp + _totalBound];
	...
	for (unsigned int i = 0; i < _nComp; ++i)
	{
		...
		for (unsigned int type = 0; type < _nParType; ++type)
		{
			...
			//AB qSum += static_cast<double>(_parTypeVolFrac[type]) * qSumType;
		}
		//AB mat.native(i, _nComp + _totalBound) += alpha * (c[i] + invBeta * qSum); 
 ...
 */
void CSTRModelVarPor::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
 ...
 * @param [in] adDirOffset Number of AD directions used for non-Jacobian purposes (e.g., parameter sensitivities)
 */
void CSTRModelVarPor::checkAnalyticJacobianAgainstAd(active const* const adRes, unsigned int adDirOffset) const
{
...
int CSTRModelVarPor::Exporter::writeMobilePhase(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}

int CSTRModelVarPor::Exporter::writeSolidPhase(double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int CSTRModelVarPor::Exporter::writeSolidPhase(unsigned int parType, double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int CSTRModelVarPor::Exporter::writeVolume(double* buffer) const
{
...
int CSTRModelVarPor::Exporter::writeInlet(double* buffer) const
{
	std::copy_n(_data, _nComp, buffer);
	return _nComp;
}

int CSTRModelVarPor::Exporter::writeOutlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
...
int CSTRModelVarPor::Exporter::writeOutlet(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}
...
{
	models[CSTRModelVarPor::identifier()] = [](UnitOpIdx uoId, IParameterProvider&) { return new CSTRModelVarPor(uoId); };
}
