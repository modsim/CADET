...
/**
* @file
* Defines the CSTR with variable porocity model as a unit operation.
*/
 ...
 * @details This is a simple CSTR with variable porocity model with variable volume using the ``well mixed assumption''.
 * @f[\begin{align}
 ...
 */
class CSTRModelVarPor : public UnitOperationBase
{
public:

	CSTRModelVarPor(UnitOpIdx unitOpIdx);
	virtual ~CSTRModelVarPor() CADET_NOEXCEPT;
	...
	static const char* identifier() { return "CSTRVarPor"; }
...
protected:
	...
	public:
		...
		// virtual int writePorosity(double vsolid,double* buffer) const { return 0; }
