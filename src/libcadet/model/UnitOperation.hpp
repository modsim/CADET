...
public:
	 ...
	 *          @f[ \frac{F}{\partial y} s + \frac{F}{\partial \dot{y}} \dot{s} + \frac{\partial F}{\partial p_i} = 0.
	 * @f]
	 * @param [in] simTime Simulation time information (time point, section index, pre-factor of time derivatives)
	 * @param [in] simState State of the simulation (state vector and its time derivative)
	 ...
	 * @param [in] adRes Pointer to local residual vector of AD datatypes with the sensitivity derivatives from
	 * residualSensFwdAdOnly()
	...
	virtual int residualSensFwdCombine(const SimulationTime& simTime, const ConstSimulationState& simState,
									   ...
									   double* const tmp2, double* const tmp3) = 0;

	/**
	 * @brief Evaluates the residuals with AD to compute the parameter sensitivities and at the same time updates the
	 * Jacobian
	 * @details Evaluates @f$ \frac{\partial F}{\partial p} @f$, where @f$ p @f$ are the sensitive parameters
	 *          and @f$ F @f$ is the residual function of this model. At the same time updates the Jacobian of the
	 * system.
	 * @param [in] simTime Simulation time information (time point, section index, pre-factor of time derivatives)
	 * @param [in] simState State of the simulation (state vector and its time derivative)
	 * @param [in,out] adJac Jacobian information for AD (AD vectors for residual and state, direction offset)
	...
	/**
	 * @brief Computes approximately / partially consistent initial values (state variables without their time
	 * derivatives)
	 * @details Given the DAE \f[ F(t, y, \dot{y}) = 0, \f] the initial values \f$ y_0 \f$ and \f$ \dot{y}_0 \f$ have
	 *          to be consistent. This functions updates the initial state \f$ y_0 \f$ and, therefore, provides the
	 *          first step in consistent initialization.
	 ...
	 * @param [in,out] vecStateYdot On entry, inconsistent state time derivatives. On exit, partially consistent state
	 * time derivatives.
	 ...
	 * @details Given the DAE \f[ F(t, y, \dot{y}, p) = 0, \f] the corresponding (linear) forward sensitivity
	 *          system reads \f[ \frac{\partial F}{\partial y}(t, y, \dot{y}) s + \frac{\partial F}{\partial \dot{y}}(t,
	 * y, \dot{y}) \dot{s} + \frac{\partial F}{\partial p}(t, y, \dot{y}) = 0. \f] The initial values of \f$ s_0 =
	 * \frac{\mathrm{d} y_0}{\mathrm{d}p} \f$ and \f$ \dot{s}_0 = \frac{\mathrm{d} \dot{y}_0}{\mathrm{d}p} \f$ have to
	 * be consistent, that means, they have to satisfy the sensitivity equation. This function computes the correct \f$
	 * s_0 \f$ and \f$ \dot{s}_0 \f$ given \f$ y_0 \f$ and \f$ s_0 \f$.
	 *
	 * @param [in] simTime Simulation time information (time point, section index, pre-factor of time derivatives)
	 * @param [in] simState State of the simulation (state vector and its time derivative)
	...
	virtual void multiplyWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState,
									  double const* yS, double alpha, double beta, double* ret) = 0;

	/**
	 * @brief Multiplies the time derivative Jacobian @f$ \frac{\partial F}{\partial \dot{y}}\left(t, y, \dot{y}\right)
	 * @f$ with a given vector
