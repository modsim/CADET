...
"confPrefix": "GIEX_"}, { "type": "ReferenceConcentrationParameter", "varName": ["refPhC0", "refPhQ"], "objName":
"refConcentrationPh", "confPrefix": "GIEX_PH", "skipConfig": true}
		]
}
</codegen>*/
...
inline const char* GIEXParamHandler::identifier() CADET_NOEXCEPT
{
	return "GENERALIZED_ION_EXCHANGE";
}
...
inline const char* ExtGIEXParamHandler::identifier() CADET_NOEXCEPT
{
	return "EXT_GENERALIZED_ION_EXCHANGE";
}
 ...
 * @details Implements the generalized ion exchange binding model, which is based on the steric mass action model: \f[
 * \begin{align}
 *              q_0 &= \Lambda - \sum_{j \geq 2} \nu_j(\mathrm{pH}) q_j \\
 *              \frac{\partial q_i}{\partial t} &= k_{a,i}(c_p, q, \mathrm{pH}) \left(\Lambda - \sum_{j \geq 2}
 * \left(\nu_j(\mathrm{pH}) + \sigma_{j}\right) q_j \right)^{\nu_i(\mathrm{pH})} c_{p,i} - k_{d,i}(c_p, q, \mathrm{pH})
 * c_{p,0}^{\nu_i(pH)} q_i \\
 *              \nu_i(\mathrm{pH}) &= \nu_{i,0} + \mathrm{pH} \nu_{i,1} + \mathrm{pH}^2 \nu_{i,2} \\
 *              k_{a,i}\left(c_p, q, \mathrm{pH}\right) &= k_{a,i,0} \exp\left(k_{a,i,1} \mathrm{pH} + k_{a,i,2}
 * \mathrm{pH}^2 + k_{a,i,\mathrm{salt}} c_{p,0} + k_{a,i,\mathrm{prot}} c_{p,i}\right) \\ k_{d,i}\left(c_p, q,
 * \mathrm{pH}\right) &= k_{d,i,0} \exp\left(k_{d,i,1} \mathrm{pH} + k_{d,i,2} \mathrm{pH}^2 + k_{d,i,\mathrm{salt}}
 * c_{p,0} + k_{d,i,\mathrm{prot}} c_{p,i}\right) \end{align} \f] Component @c 0 is assumed to be salt. Component @c 1
 * is a second non-binding modifier component (e.g., pH). Multiple bound states are not supported. Components without
 * bound state (i.e., non-binding components) are supported.
 *
 *          See @cite Huuk2017.
...
public:
											  ...
											  unsigned int const* nBound, unsigned int const* boundOffset)
	{
		...
		if (nBound[0] != 1)
			throw InvalidParameterException(
				"Generalized ion exchange binding model requires exactly one bound state for salt component");

		// Guarantee that modifier component is non-binding
		if (nBound[1] != 0)
			throw InvalidParameterException(
				"Generalized ion exchange binding model requires non-binding modifier component (NBOUND[1] = 0)");
										   ...
										   double const* yCp, LinearBufferAllocator workSpace) const
	{
		...
		for (int j = 2; j < _nComp; ++j)
		{
			...
			const double nu_j = static_cast<double>(p->nu[j]) +
								pH * (static_cast<double>(p->nuLin[j]) + pH * static_cast<double>(p->nuQuad[j]));
...
protected:
	...
	virtual bool configureImpl(IParameterProvider& paramProvider, UnitOpIdx unitOpIdx, ParticleTypeIdx parTypeIdx)
	{
		...
		if (paramProvider.exists(std::string(_paramHandler.prefixInConfiguration()) + "GIEX_PHREFC0") &&
			paramProvider.exists(std::string(_paramHandler.prefixInConfiguration()) + "GIEX_PHREFQ"))
		{
				 ...
				 CpStateType const* yCp, ResidualType* res, LinearBufferAllocator workSpace) const
	{
		...
		for (int j = 2; j < _nComp; ++j)
		{
			...
			const CpStateParamType nu_j =
				static_cast<ParamType>(p->nu[j]) +
				pH * (static_cast<ParamType>(p->nuLin[j]) + pH * static_cast<ParamType>(p->nuQuad[j]));
		...
		for (int i = 2; i < _nComp; ++i)
		{
			...
			//			const CpStateParamType nu_i_over_nu0 = (static_cast<ParamType>(p->nu[i]) + pH *
			//(static_cast<ParamType>(p->nuLin[i]) + pH * static_cast<ParamType>(p->nuQuad[i]))) /
			// static_cast<ParamType>(p->nu[0]); 			const CpStateParamType c0_pow_nu = pow(yCp0_divRef,
			// nu_i_over_nu0); 			const StateParamType q0_bar_pow_nu = pow(q0_bar_divRef, nu_i_over_nu0);
			const CpStateParamType nu_i_0_over_nu0 =
				static_cast<ParamType>(p->nu[i]) / static_cast<ParamType>(p->nu[0]);
			const CpStateParamType nu_i_pH_over_nu0 =
				pH * (static_cast<ParamType>(p->nuLin[i]) + pH * static_cast<ParamType>(p->nuQuad[i])) /
				static_cast<ParamType>(p->nu[0]);
			const CpStateParamType c0_pow_nu =
				pow(yCp0_divRef, nu_i_0_over_nu0) * pow(yCp0_ph_divRef, nu_i_pH_over_nu0);
			const StateParamType q0_bar_pow_nu =
				pow(q0_bar_divRef, nu_i_0_over_nu0) * pow(q0_bar_ph_divRef, nu_i_pH_over_nu0);

			// k_{a,i}(c_p, q, \mathrm{pH}) = k_{a,i,0} \exp(k_{a,i,1} \mathrm{pH} + k_{a,i,2} \mathrm{pH}^2 +
			// k_{a,i,\mathrm{salt}} c_{p,0} + k_{a,i,\mathrm{prot}} c_{p,i})
			const CpStateParamType ka_i =
			...
			const CpStateParamType kd_i =
				static_cast<ParamType>(p->kD[i]) *
					  ...
					  int offsetCp, RowIterator jac, LinearBufferAllocator workSpace) const
	{
		...
		// Getting to c_{p,0}: -bndIdx takes us to q_0, another -offsetCp to c_{p,0}. This means jac[-bndIdx - offsetCp]
		// corresponds to c_{p,0}. Getting to c_{p,i}: -bndIdx takes us to q_0, another -offsetCp to c_{p,0} and a +i to
		// c_{p,i}.
		...
		for (int j = 2; j < _nComp; ++j)
		{
			...
			jac[bndIdx] = nu_j;
			jac[1 - offsetCp] +=
				(static_cast<double>(p->nuLin[j]) + 2.0 * pH * static_cast<double>(p->nuQuad[j])) * y[bndIdx];
		...
		for (int i = 2; i < _nComp; ++i)
		{
			...
			const double nu_pH = pH * (static_cast<double>(p->nuLin[i]) + pH * static_cast<double>(p->nuQuad[i])) /
								 static_cast<double>(p->nu[0]);
			...
			const double q0_bar_pow_nu_m1_divRef =
				nu * pow(q0_bar_divRef, nu_0 - 1.0) * pow(q0_bar_ph_divRef, nu_pH) / refQ;

			// k_{a,i}(c_p, q, \mathrm{pH}) = k_{a,i,0} \exp(k_{a,i,1} \mathrm{pH} + k_{a,i,2} \mathrm{pH}^2 +
			// k_{a,i,\mathrm{salt}} c_{p,0} + k_{a,i,\mathrm{prot}} c_{p,i})
			const double ka_i =
				ka * exp(pH * (static_cast<double>(p->kALin[i]) + pH * static_cast<double>(p->kAQuad[i])) +
						 static_cast<double>(p->kASalt[i]) * yCp0_divRef + static_cast<double>(p->kAProt[i]) * yCp[i]);
			const double dKaDpH =
				ka_i * (static_cast<double>(p->kALin[i]) + 2.0 * pH * static_cast<double>(p->kAQuad[i]));
			const double kd_i =
				kd * exp(pH * (static_cast<double>(p->kDLin[i]) + pH * static_cast<double>(p->kDQuad[i])) +
						 static_cast<double>(p->kDSalt[i]) * yCp0_divRef + static_cast<double>(p->kDProt[i]) * yCp[i]);
			const double dKdDpH =
				kd_i * (static_cast<double>(p->kDLin[i]) + 2.0 * pH * static_cast<double>(p->kDQuad[i]));

			// dres_i / dc_{p,0}
			jac[-bndIdx - offsetCp] =
				kd_i * y[bndIdx] * (c0_pow_nu_m1_divRef + c0_pow_nu * static_cast<double>(p->kDSalt[i]) / refC0) -
				ka_i * yCp[i] * q0_bar_pow_nu * static_cast<double>(p->kASalt[i]) / refC0;
			// dres_i / dc_{p,1}
			jac[1 - bndIdx - offsetCp] = y[bndIdx] * c0_pow_nu * (dKdDpH + kd_i * std::log(yCp0_ph_divRef) * dNuDpH) -
										 yCp[i] * q0_bar_pow_nu * (dKaDpH + ka_i * std::log(q0_bar_ph_divRef) * dNuDpH);
			// dres_i / dc_{p,i}
			jac[i - bndIdx - offsetCp] = -ka_i * q0_bar_pow_nu * (1.0 + yCp[i] * static_cast<double>(p->kAProt[i])) +
										 kd_i * y[bndIdx] * c0_pow_nu * static_cast<double>(p->kDProt[i]);
			// dres_i / dq_0
	...
	std::unordered_map<std::string, std::function<model::IBindingModel*()>>& bindings)
{
	...
	bindings[ExternalGeneralizedIonExchangeBinding::identifier()] = []() {
		return new ExternalGeneralizedIonExchangeBinding();
	};
}
} // namespace binding
