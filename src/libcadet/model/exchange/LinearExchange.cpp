...
public:
	...
	LinearExchangeBase() : _nComp(0), _nBoundStates(nullptr), _reactionQuasistationarity(0, false) { }
	...
	virtual bool configureModelDiscretization(IParameterProvider& paramProvider, unsigned int nComp, unsigned int const* nBound, unsigned int const* boundOffset)
	{
		...
		{
			const bool kineticExchange = paramProvider.getInt("IS_KINETIC");
			std::fill(_reactionQuasistationarity.begin(), _reactionQuasistationarity.end(), !kineticExchange);
		}
...
namespace exchange
{
	void registerLinearModel(std::unordered_map<std::string, std::function<model::IExchangeModel*()>>& exchange)
	{
		exchange[LinearExchange::identifier()] = []() { return new LinearExchange(); };
		exchange[ExternalLinearEchange::identifier()] = []() { return new ExternalLinearExchange(); };
	}
}  // namespace exchange
