...
template <typename T> struct FlowParameters
{
	T u;
	active const* d_ax;
	T h;
...
int residualForwardsFlow(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
						 RowIteratorType jacBegin, const FlowParameters<ParamType>& p)
{
	...
	for (unsigned int comp = 0; comp < p.nComp; ++comp)
	{
		...
		for (unsigned int col = 0; col < p.nCol; ++col)
		{
			...
			if (cadet_likely(col > 0))
			{
				...
				if (wantJac)
				{
					for (int i = 0; i < 2 * wenoOrder - 1; ++i)
						...
						// cell)
						jac[(i - wenoOrder) * p.strideCell] -=
							static_cast<double>(p.u) / static_cast<double>(p.h) * p.wenoDerivatives[i];
				}
			}
			else
			{
			...
			if (wantJac)
				wenoOrder = p.weno->template reconstruct<StateType, StencilType>(p.wenoEpsilon, col, p.nCol, stencil,
																				 vm, p.wenoDerivatives);
			else
				wenoOrder =
					p.weno->template reconstruct<StateType, StencilType>(p.wenoEpsilon, col, p.nCol, stencil, vm);
			...
			{
				for (int i = 0; i < 2 * wenoOrder - 1; ++i)
					jac[(i - wenoOrder + 1) * p.strideCell] +=
						static_cast<double>(p.u) / static_cast<double>(p.h) * p.wenoDerivatives[i];
			}
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac>
int residualBackwardsFlow(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
						  RowIteratorType jacBegin, const FlowParameters<ParamType>& p)
{
	const ParamType h2 = p.h * p.h;
	...
	for (unsigned int comp = 0; comp < p.nComp; ++comp)
	{
		...
		for (unsigned int col = p.nCol - 1; col < p.nCol; --col)
		{
			...
			{
				for (int i = 0; i < 2 * wenoOrder - 1; ++i)
					jac[(wenoOrder - i - 1) * p.strideCell] -=
						static_cast<double>(p.u) / static_cast<double>(p.h) * p.wenoDerivatives[i];
			}

			// Update stencil (be careful because of wrap-around, might cause reading memory very far away [although
			// never used])
			const unsigned int shift = std::max(p.weno->order(), 2);
			if (cadet_likely(col - shift < p.nCol))
				stencil.advance(yBulkComp[(col - shift) * p.strideCell]);
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac>
int residualKernel(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
				   RowIteratorType jacBegin, const FlowParameters<ParamType>& p)
{
	if (p.u >= 0.0)
		return impl::residualForwardsFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac>(
			simTime, y, yDot, res, jacBegin, p);
	else
		return impl::residualBackwardsFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac>(
			simTime, y, yDot, res, jacBegin, p);
}

void sparsityPattern(linalg::SparsityPatternRowIterator itBegin, unsigned int nComp, unsigned int nCol, int strideCell,
					 double u, Weno& weno);
