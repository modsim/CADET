...
\end{align} @f]
 * Methods are described in @cite VonLieres2010a (WENO, linear solver), and @cite Puttmann2013, @cite Puttmann2016
(forward sensitivities, AD, band compression)
 *
...
public:
	...
	bool configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
				   std::unordered_map<ParameterId, active*>& parameters);
	bool notifyDiscontinuousSectionTransition(double t, unsigned int secIdx);

	int residual(double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
				 linalg::BandMatrix& jac);
	int residual(double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 linalg::BandMatrix& jac);
	int residual(double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
	...
	int residual(double t, unsigned int secIdx, active const* y, double const* yDot, active* res,
				 WithoutParamSensitivity);
	...
	inline const active& columnLength() const CADET_NOEXCEPT
	{
		return _colLength;
	}
	inline const active& crossSectionArea() const CADET_NOEXCEPT
	{
		return _crossSection;
	}
	inline const active& currentVelocity() const CADET_NOEXCEPT
	{
		return _curVelocity;
	}

	inline unsigned int nComp() const CADET_NOEXCEPT
	{
		return _nComp;
	}
	inline unsigned int nCol() const CADET_NOEXCEPT
	{
		return _nCol;
	}
	inline const Weno& weno() const CADET_NOEXCEPT
	{
		return _weno;
	}
	...
	bool setSensitiveParameter(std::unordered_set<active*>& sensParams, const ParameterId& pId,
							   unsigned int adDirection, double adValue);
...
protected:
	...
	int residualImpl(double t, unsigned int secIdx, StateType const* y, double const* yDot, ResidualType* res,
					 RowIteratorType jacBegin);

	template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac>
	int residualForwardsFlow(double t, unsigned int secIdx, StateType const* y, double const* yDot, ResidualType* res,
							 RowIteratorType jacBegin);

	template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac>
	int residualBackwardsFlow(double t, unsigned int secIdx, StateType const* y, double const* yDot, ResidualType* res,
							  RowIteratorType jacBegin);
	...
	{
		return static_cast<int>(_strideCell);
	}
...
public:
	...
	int residual(double t, unsigned int secIdx, double const* y, double const* yDot, double* res, bool wantJac,
				 WithoutParamSensitivity);
	int residual(double t, unsigned int secIdx, active const* y, double const* yDot, active* res, bool wantJac,
				 WithParamSensitivity);
	int residual(double t, unsigned int secIdx, active const* y, double const* yDot, active* res, bool wantJac,
				 WithoutParamSensitivity);
	int residual(double t, unsigned int secIdx, double const* y, double const* yDot, active* res, bool wantJac,
				 WithParamSensitivity);
	...
	inline const active& columnLength() const CADET_NOEXCEPT
	{
		return _baseOp.columnLength();
	}
	inline const active& crossSectionArea() const CADET_NOEXCEPT
	{
		return _baseOp.crossSectionArea();
	}
	inline const active& currentVelocity() const CADET_NOEXCEPT
	{
		return _baseOp.currentVelocity();
	}

	inline linalg::BandMatrix& jacobian() CADET_NOEXCEPT
	{
		return _jacC;
	}
	inline const linalg::BandMatrix& jacobian() const CADET_NOEXCEPT
	{
		return _jacC;
	}

	inline linalg::FactorizableBandMatrix& jacobianDisc() CADET_NOEXCEPT
	{
		return _jacCdisc;
	}
	inline const linalg::FactorizableBandMatrix& jacobianDisc() const CADET_NOEXCEPT
	{
		return _jacCdisc;
	}
	...
	inline bool setSensitiveParameter(std::unordered_set<active*>& sensParams, const ParameterId& pId,
									  unsigned int adDirection, double adValue)
	{
		return _baseOp.setSensitiveParameter(sensParams, pId, adDirection, adValue);
	}
	inline bool setSensitiveParameterValue(const std::unordered_set<active*>& sensParams, const ParameterId& id,
										   double value)
	{
