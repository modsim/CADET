 ...
 *
 * @f[\begin{align}
	\frac{\partial c_i}{\partial t} &= - u \frac{\partial c_i}{\partial z} + D_{\text{ax},i}(\rho) \frac{\partial^2
c_i}{\partial z^2} + \frac{1}{\rho} \frac{\partial}{\partial \rho} \left( \rho D_{\rho} \frac{\partial c_i}{\partial
\rho} \right) \\ \end{align} @f]
...
public:
	...
	inline const active& columnRadius() const CADET_NOEXCEPT
	{
		return _colRadius;
	}
	inline const active& currentVelocity(int idx) const CADET_NOEXCEPT
	{
		return _curVelocity[idx];
	}
	inline const active& columnPorosity(int idx) const CADET_NOEXCEPT
	{
		return _colPorosities[idx];
	}
	inline const active& crossSection(int idx) const CADET_NOEXCEPT
	{
		return _crossSections[idx];
	}
	inline active const* crossSections() const CADET_NOEXCEPT
	{
		return _crossSections.data();
	}
	inline active const* radialCenters() const CADET_NOEXCEPT
	{
		return _radialCenters.data();
	}
	//	inline active const* radialCentroids() const CADET_NOEXCEPT { return _radialCentroids.data(); }
	inline active const* radialEdges() const CADET_NOEXCEPT
	{
		return _radialEdges.data();
	}
	inline bool isCurrentFlowForward(int idx) const CADET_NOEXCEPT
	{
		return _curVelocity[idx] >= 0.0;
	}
	...
	inline linalg::CompressedSparseMatrix& jacobian() CADET_NOEXCEPT
	{
		return _jacC;
	}
	inline const linalg::CompressedSparseMatrix& jacobian() const CADET_NOEXCEPT
	{
		return _jacC;
	}
...
protected:
	...
	bool _hasDynamicReactions; //!< Determines whether the model has dynamic reactions (only relevant for sparsity
							   //!< pattern)
