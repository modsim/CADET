...
MatrixXd TwoDimensionalConvectionDispersionOperatorDG::calcTildeMr(const unsigned int elemIdx)
{
...
MatrixXd TwoDimensionalConvectionDispersionOperatorDG::calcTildeSrDash(const unsigned int elemIdx)
{
	MatrixXd ellEll = MatrixXd::Zero(_radNNodes, _qNNodes);
...
void TwoDimensionalConvectionDispersionOperatorDG::initializeDG()
{
	...
	// operators
	_axInvMM = dgtoolbox::invMMatrix(_axPolyDeg, _axNodes, 0.0, 0.0);
...
{
	// todo return tripletlist, Jacobian will be owned by unit operation
}
 ...
 * @param [in] bulkOffset Offset to first entry of bulk phase in unit Jacobian
 */
