...
} : public cadet::model::ConstParamHandlerBase
{
public:
	struct ConstParams
	{
	...
	_{{p / objName}}({ % for v in p / varName % } & _localParams.{{v}} { % if not is_last % },
					 { % endif % } { % endfor % }){ % else % } _{{p / varName}}(&_localParams.{{p / varName}}){
		% endif % }
	{
		% if not is_last %
	}
	...
	inline bool configure(IParameterProvider & paramProvider, unsigned int nReactions, unsigned int nComp,
						  unsigned int const* nBoundStates)
	{
		...
		{ % else % } _{{p / objName}}.configure("{{ p/confPrefix }}", paramProvider, nComp, nBoundStates);
		{ % endif % } { % else % } {
			% if existsIn (p, "default") %
		}
		_{{p / varName}}.configure("{{ p/confName }}", paramProvider, nComp, nBoundStates, {{p / default}});
		{ % else % } _{{p / varName}}.configure("{{ p/confName }}", paramProvider, nComp, nBoundStates);
								   ...
								   ParticleTypeIdx parTypeIdx, unsigned int nComp, unsigned int const* nBoundStates)
	{
		...
		{ % else % } _{{p / varName}}.registerParam("{{ p/confName }}", parameters, unitOpIdx, parTypeIdx, nComp,
													nBoundStates);
		{ % endif % } { % endfor % } {
			% endif %
		}
	...
	inline void reserve(unsigned int nReactions, unsigned int nComp, unsigned int nBoundStates)
	{
		...
		{ % else % } _{{p / varName}}.reserve(nReactions, nComp, nBoundStates);
		{ % endif % } { % endfor % } {
			% endif %
		}
	}
	...
	inline std::tuple<ParamsHandle, ParamsHandle> updateTimeDerivative(
		double t, unsigned int secIdx, const ColumnPosition& colPos, unsigned int nComp,
		unsigned int const* nBoundStates, LinearBufferAllocator& workSpace) const
	{
	...
	inline const {{p / type}} & {{p / varName}}() const CADET_NOEXCEPT
	{
		return _{{p / varName}};
	}
	inline {{p / type}} & {{p / varName}}() CADET_NOEXCEPT
	{
		return _{{p / varName}};
	}
	{ % endfor % } {
	...
	inline const {{p / type}} & {{p / objName}}() const CADET_NOEXCEPT
	{
		return _{{p / objName}};
	}
	inline {{p / type}} & {{p / objName}}() CADET_NOEXCEPT
	{
		return _{{p / objName}};
	}
	{ % endif % } { % endfor % } {
	...
	inline char const* prefixInConfiguration() const CADET_NOEXCEPT
	{
		return "";
	}
...
} : public cadet::model::ExternalParamHandlerBase
{
public:
	struct ConstParams
	{
	...
	struct VariableParams
	{
		...
		typename util::localVersionOf <
		{
			{
				p / type
			}
		}
		::storage_t > ::type{{p / varName}};
		{ % endfor % } {
			% if exists ("constantParameters") %
		}
		{% for p in constantParameters %
	...
	{{externalName}}() CADET_NOEXCEPT
	{
		...
		_{{p / objName}}({ % for v in p / varName % } & _constParams.{{v}} { % if not is_last % },
						 { % endif % } { % endfor % }){ % else % } _{{p / varName}}(&_constParams.{{p / varName}}){
			% endif % }
		{
			% if not is_last %
		}
							  ...
							  unsigned int const* nBoundStates)
		{
			...
			_{{p / varName}}.configure("{{ p/confName }}", paramProvider, nComp, nBoundStates, {{p / default}});
			{ % else % } _{{p / varName}}.configure("{{ p/confName }}", paramProvider, nComp, nBoundStates);
			{ % endif % } { % endif % } { % endif % } { % endfor % } { % endif % } ExternalParamHandlerBase::configure(
				paramProvider, {{length(parameters)}});
			return validateConfig(nReactions, nComp, nBoundStates);
		}
									   ...
									   ParticleTypeIdx parTypeIdx, unsigned int nComp, unsigned int const* nBoundStates)
		{
			...
			_{{p / objName}}.registerParam("{{ p/confPrefix }}", parameters, unitOpIdx, parTypeIdx, nComp,
										   nBoundStates);
			{ % else % } _{{p / varName}}.registerParam("{{ p/confName }}", parameters, unitOpIdx, parTypeIdx, nComp,
														nBoundStates);
			{ % endif % } { % endfor % } {
								   ...
								   unsigned int const* nBoundStates, LinearBufferAllocator& workSpace) const
		{
			...
			{ % endfor % } { % else % } localParams->{{p / varName}} = _constParams.{{p / varName}};
			{ % endif % } { % endfor % } { % endif % }

			{% for p in parameters %
			}
			_{{p / varName}}.prepareCache(localParams->{{p / varName}}, workSpace);
			_{{p / varName}}.update(cadet::util::dataOfLocalVersion(localParams->{{p / varName}}),
									extFunBuffer[{{index}}], nComp, nBoundStates);
			{
				% endfor %
			}
			...
			unsigned int const* nBoundStates, LinearBufferAllocator& workSpace) const
		{
			...
			evaluateExternalFunctions(t, secIdx, colPos, {{length(parameters)}}, static_cast<double*>(extFunBuffer));
			evaluateTimeDerivativeExternalFunctions(t, secIdx, colPos, {{length(parameters)}},
													static_cast<double*>(extDerivBuffer));
			...
			_{{p / varName}}.updateTimeDerivative(cadet::util::dataOfLocalVersion(p->{{p / varName}}),
												  extFunBuffer[{{index}}], extDerivBuffer[{{index}}], nComp,
												  nBoundStates);
			{
				% endfor %
			}
		...
		inline std::size_t cacheSize(unsigned int nReactions, unsigned int nComp, unsigned int totalNumBoundStates)
			const CADET_NOEXCEPT
		{
			return 2 * sizeof(params_t) + alignof(params_t) + 2 * {{length(parameters)}} * sizeof(double) +
				   alignof(double) +
				   2 * ({ % for p in parameters % } _{{p / varName}}.additionalDynamicMemory(nReactions, nComp,
																							 totalNumBoundStates) {
					   % if not is_last %
				   } + { % endif % } { % endfor % });
		}
		...
		inline const External{{p / type}} & {{p / varName}}() const CADET_NOEXCEPT
		{
			return _{{p / varName}};
		}
		inline External{{p / type}} & {{p / varName}}() CADET_NOEXCEPT
		{
			return _{{p / varName}};
		}
		{ % endfor % } {
