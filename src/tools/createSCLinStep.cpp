...
int main(int argc, char** argv)
{
	...
	try
	{
		TCLAP::CustomOutputWithoutVersion customOut("createSCLstep");
		TCLAP::CmdLine cmd(
			"Create an HDF5 input file for a single component linear benchmark case with pulse injection", ' ', "1.0");
		cmd.setOutput(&customOut);

		cmd >> (new TCLAP::ValueArg<std::string>("o", "out", "Write output to file (default: SCLinStep.h5)", false,
												 "SCLinStep.h5", "File"))
	...
	// Solver
	{
		...
		// Sections
		{
			...
			const double secTimes[] = {0.0, 10.0, 200.0};
