...
TEST_CASE("GRM LWE forward vs backward flow", "[GRM],[FV],[Simulation]")
{
...
TEST_CASE("GRM linear pulse vs analytic solution", "[GRM],[FV],[Simulation],[Analytic]")
{
...
TEST_CASE("GRM non-binding linear pulse vs analytic solution", "[GRM],[FV],[Simulation],[Analytic],[NonBinding]")
{
	cadet::test::column::FVparams disc(512);
...
TEST_CASE("GRM Jacobian forward vs backward flow", "[GRM],[UnitOp],[Residual],[Jacobian],[AD]")
{
	// Test all WENO orders
	for (unsigned int i = 1; i <= cadet::Weno::maxOrder(); ++i)
		cadet::test::column::testJacobianWenoForwardBackward("GENERAL_RATE_MODEL", "FV", i);
}
...
TEST_CASE("GRM forward sensitivity vs FD", "[GRM],[Sensitivity],[Simulation]")
{
	...
	cadet::test::column::testFwdSensSolutionFD("GENERAL_RATE_MODEL", "FV", false, fdStepSize, absTols, relTols, passRatio);
}
...
TEST_CASE("GRM forward sensitivity forward vs backward flow", "[GRM],[Sensitivity],[Simulation]")
{
	...
	cadet::test::column::testFwdSensSolutionForwardBackward("GENERAL_RATE_MODEL", "FV", absTols, relTols, passRatio);
}

TEST_CASE("GRM consistent initialization with linear binding", "[GRM],[ConsistentInit]")
{
	cadet::test::column::testConsistentInitializationLinearBinding("GENERAL_RATE_MODEL", "FV", 1e-12, 1e-14);
}
...
//		1.0, 1.8, 1.5, 1.6, 856.173, 64.457, 5.73227, 2.85286};
	...
	cadet::test::column::testConsistentInitializationSMABinding("GENERAL_RATE_MODEL", "FV", y.data(), 1e-14, 1e-5);
}
