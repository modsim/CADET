...
{
public:
	ConfiguredParameterDependence(ConfiguredParameterDependence&& cpy) CADET_NOEXCEPT : _paramDep(cpy._paramDep),
																						_nComp(cpy._nComp),
																						_nBound(cpy._nBound),
	...
	static ConfiguredParameterDependence create(const char* name, unsigned int nComp, unsigned int const* nBound,
												const char* config);

	inline cadet::model::IParameterDependence& model()
	{
		return *_paramDep;
	}
	inline const cadet::model::IParameterDependence& model() const
	{
		return *_paramDep;
	}
...
private:
	ConfiguredParameterDependence(cadet::model::IParameterDependence* paramDep, unsigned int nComp,
