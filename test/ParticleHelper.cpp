...
namespace
{
template <typename T> void replicateData(std::vector<T>& data, unsigned int nTimes)
{
...
void replicateFieldDataDouble(cadet::JsonParameterProvider& jpp, const std::string& field,
...
void extendModelToManyParticleTypesImpl(cadet::JsonParameterProvider& jpp, const factor_t& factors, unsigned int nTypes,
...
void extendModelToManyParticleTypes(cadet::JsonParameterProvider& jpp, const std::vector<double>& factors,
									double const* const volFrac)
{
...
void testLinearMixedParticleTypesImpl(cadet::JsonParameterProvider& jpp, double absTol, double relTol,
									  modifier_t modify)
{
	for (int i = 0; i < 2; ++i)
	{
...
{
	cadet::JsonParameterProvider jpp = createPulseInjectionColumn(uoType, true);
	testLinearMixedParticleTypesImpl(
		jpp, absTol, relTol, [](cadet::JsonParameterProvider& jpp) { scrambleParticleTypeFractionsSpatially(jpp, 3); });
}
