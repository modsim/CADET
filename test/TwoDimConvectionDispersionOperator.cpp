...
inline void createAndConfigureOperator(cadet::model::parts::TwoDimensionalConvectionDispersionOperator& convDispOp,
									   int nComp, int nCol, int nRad, int wenoOrder)
{
...
inline void compareSparseJacobianAgainstFD(cadet::model::parts::TwoDimensionalConvectionDispersionOperator& convDispOp,
										   int nInletDof, int nPureDof, double* y, double* jacCol1, double* jacCol2,
...
void testBulk2DJacobianWenoForwardBackward(int wenoOrder)
{
	...
	SECTION("Forward vs backward flow Jacobian (WENO=" + std::to_string(wenoOrder) + ")")
	{
		...
		cadet::test::util::populate(
			y.data() + nInletDof,
			[](unsigned int idx) { return std::abs(std::sin(idx * 0.13)) + std::abs(std::sin(idx * 0.3)) + 1e-4; },
			nPureDof);
		...
		compareSparseJacobianAgainstFD(convDispOp, nInletDof, nPureDof, y.data(), jacCol1.data(), jacCol2.data(), h,
									   relTol, absTol);
...
TEST_CASE("TwoDimensionalConvectionDispersionOperator Jacobian forward vs backward flow",
		  "[2D],[Operator],[Residual],[Jacobian]")
{
...
TEST_CASE("TwoDimensionalConvectionDispersionOperator Jacobian sparsity pattern vs FD",
		  "[2D],[Operator],[Residual],[Jacobian],[SparseMatrix]")
{
